...
test('a public browser-only instance says where evidence goes and reads the linked lab to its ground truth', async ({ page, request }) => {
  ...
  await page.goto('/')
  // a first visit opens on the home page, which says where files go and offers no server store
  await expect(page.getByRole('heading', { name: /^Detection and investigation/ })).toBeVisible()
  await expect(page.getByText(/Files are parsed on remn\.localhost, which keeps nothing/)).toBeVisible()
  await page.getByRole('button', { name: 'New case' }).first().click()
  await expect(page.getByRole('radio', { name: /Browser/ })).toBeChecked()
  await expect(page.getByRole('radio', { name: /Server/ })).toHaveCount(0)
