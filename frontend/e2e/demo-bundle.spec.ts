 ...
 * configuration, the rules run, the stories are read (and the chains with them), and the case bundle
 * the app exports is what the demo button restores. The bundle is therefore what a visitor's own browser would have
...
const storyLine = (page: Page) => page.locator('.view-header .sub').filter({ hasText: /stor(y|ies) ·/ })
...
test('writes the demo case from the linked lab, as this app reads it', async ({ page }, testInfo) => {
  ...
  await expect(storyLine(page)).toContainText('6 stories · 6 critical', { timeout: 180_000 })
...
test('the demo case opens in the browser, with nothing uploaded', async ({ page }) => {
  ...
  await expect(storyLine(page)).toContainText('6 stories · 6 critical', { timeout: 120_000 })
