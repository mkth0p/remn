...
function eventFilter(e: EntityRef): Filter | null {
  ...
  switch (e.kind) {
    case 'user': {
      ...
      return { conditions: [{ field: 'targetUser', op: 'eq', value: local }, { field: 'subjectUser', op: 'eq', value: local }, { field: 'upn', op: 'eq', value: v }, { field: 'targetUser', op: 'eq', value: v }], logic: 'or', sort: { field: 'ts', dir: 'desc' } }
    }
    case 'host':
      return { conditions: [{ field: 'computer', op: 'startswith', value: v.split('.')[0] }], sort: { field: 'ts', dir: 'desc' } }
    case 'ip':
      return { conditions: [{ field: 'ipAddress', op: 'eq', value: v }, { field: 'sourceIp', op: 'eq', value: v }, { field: 'destinationIp', op: 'eq', value: v }], logic: 'or', sort: { field: 'ts', dir: 'desc' } }
    case 'domain':
      return { conditions: [{ field: 'query', op: 'endswith', value: v }, { field: 'destinationHostname', op: 'endswith', value: v }], logic: 'or', sort: { field: 'ts', dir: 'desc' } }
    default:
      return null
  }
}
function mailFilter(e: EntityRef): Filter | null {
  ...
  switch (e.kind) {
    case 'address':
      return { conditions: [{ field: 'fromAddr', op: 'eq', value: v }, { field: 'to.addr', op: 'eq', value: v }, { field: 'replyTo.addr', op: 'eq', value: v }], logic: 'or', sort: { field: 'date', dir: 'desc' } }
    case 'user':
      return v.includes('@') ? { conditions: [{ field: 'fromAddr', op: 'eq', value: v }, { field: 'to.addr', op: 'eq', value: v }], logic: 'or', sort: { field: 'date', dir: 'desc' } } : { conditions: [{ field: 'to.addr', op: 'startswith', value: v + '@' }, { field: 'fromAddr', op: 'startswith', value: v + '@' }], logic: 'or', sort: { field: 'date', dir: 'desc' } }
    case 'domain':
      return { conditions: [{ field: 'fromRegistrable', op: 'eq', value: v }, { field: 'fromDomain', op: 'eq', value: v }, { field: 'urls.domain', op: 'eq', value: v }], logic: 'or', sort: { field: 'date', dir: 'desc' } }
    case 'ip':
...
export function EntityPanel() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const [ev, ml, fs] = await Promise.all([
        ef ? ds.searchEvents(ef, 300).then((r) => r.rows).catch(() => []) : Promise.resolve([]),
        mf ? ds.searchMails(mf, 300).then((r) => r.rows).catch(() => []) : Promise.resolve([]),
        getDb().findings.where('caseId').equals(kase.id!).filter((f) => Object.values(f.entities).some((x) => { const s = String(x).toLowerCase(); return s === v || s.split(/,\s*/).includes(v) || (entity.kind === 'user' && (s === local || s.endsWith('\\' + local) || s.startsWith(local + '@'))) })).toArray(),
      ])
      if (!alive) return
      setEvents(ev)
      setMails(ml)
      ...
      if (ef) {
        ...
        const out = await Promise.all(fields.map(async (field) => ({ field, groups: await ds.aggregateEvents(ef, field, 6).then((a) => a.groups.map((g) => ({ key: String(g.value), count: g.count }))).catch(() => []) })))
  ...
  return (
    <Flyout
      ...
      tabs={<Tabs tabs={[{ id: 'timeline', label: <span>Timeline <span className="n">{items.length}</span></span> }, { id: 'insights', label: 'Insights' }, { id: 'events', label: <span>Events <span className="n">{events.length}</span></span> }, { id: 'mails', label: <span>Mails <span className="n">{mails.length}</span></span> }]} active={tab} onChange={setTab} />}
      onClose={close}
      footer={<>
        {ef && <button className="btn sm" onClick={() => { setEventsFilter(ef); setView('events'); close() }}>filter events</button>}
        {mf && <button className="btn sm" onClick={() => { setMailsFilter(mf); setView('mails'); close() }}>filter mails</button>}
        <span className="spacer" />
        <button className="btn sm" onClick={ask}><IconAi /> ask the analyst</button>
      </>}
    >
      {tab === 'timeline' && (
        <div className="story">
          ...
          {items.slice(0, 300).map((it, i) => (
            <div key={i} className="step" onClick={it.open} style={{ gridTemplateColumns: '150px 14px 1fr' }}>
              ...
              <span>
                <div className="title">{it.title}{it.kind === 'finding' && <Badge sev={it.sev} className="small" title="finding">finding</Badge>}</div>
                {it.sub && <div className="sub">{it.sub}</div>}
              </span>
            </div>
          ))}
          {items.length > 300 && <div className="muted small" style={{ padding: 8 }}>{fmtNum(items.length - 300)} more - use the filter buttons below</div>}
        </div>
      )}
      {tab === 'insights' && (
        <>
          <div className="section">
            ...
            {findings.slice(0, 20).map((f) => <div key={f.id} className="row" style={{ gap: 8 }}><Sev sev={f.severity} /><span className="ellipsis" style={{ flex: 1 }}>{f.title}</span><span className="mono small muted">{fmtTs(f.ts)}</span></div>)}
          </div>
          {aggs.map((a) => (
            <div className="section" key={a.field}>
              <h3>{a.field} seen with this {entity.kind}</h3>
              <div className="kv">{a.groups.map((g) => <div key={g.key} style={{ display: 'contents' }}><div className="k">{g.key || '(empty)'}</div><div className="v">{fmtNum(g.count)}</div></div>)}</div>
            </div>
          ))}
          {mails.length > 0 && (
            <div className="section">
              <h3>Mail</h3>
              <div className="kv">
                <div className="k">highest risk</div><div className="v">{Math.max(...mails.map((m) => m.risk))}</div>
                <div className="k">senders</div><div className="v">{Array.from(new Set(mails.map((m) => m.fromAddr))).slice(0, 6).join(', ')}</div>
                <div className="k">history</div><div className="v">{(() => { const m = mails.find((x) => x.senderPrevalence); return m ? `${m.senderPrevalence} sender · ${m.senderPriorCount ?? 0} prior mail(s) · ${m.senderSolicited === true ? 'solicited' : m.senderSolicited === false ? 'unsolicited' : 'history unknown'}` : 'run "baseline senders" for sender history' })()}</div>
              </div>
            </div>
          )}
        </>
      ...
      {tab === 'events' && (
        <table className="table compact">
          <thead><tr><th>time (UTC)</th><th>id</th><th>computer</th><th>ip</th><th>summary</th></tr></thead>
          <tbody>
            {events.slice(0, 200).map((e) => (
              <tr key={e.id} onClick={() => { setFocus({ source: 'events', id: e.id! }); setView('events'); close() }} style={{ cursor: 'pointer' }}>
                <td className="nowrap">{fmtTs(e.ts)}</td><td>{String(e.eventId ?? e.operation ?? '')}</td><td>{String(e.computer ?? '')}</td><td>{String(e.ipAddress ?? '')}</td><td className="sans">{String(e.summary ?? '')}</td>
              </tr>
            ))}
            {!events.length && <tr><td colSpan={5} className="muted sans">no events</td></tr>}
          </tbody>
        </table>
      )}
      {tab === 'mails' && (
        <table className="table compact">
          ...
          <tbody>
            {mails.slice(0, 200).map((m) => (
              <tr key={m.id} onClick={() => { setFocus({ source: 'mails', id: m.id! }); setView('mails'); close() }} style={{ cursor: 'pointer' }}>
                <td className="nowrap">{fmtTs(m.date)}</td><td>{m.risk}</td><td>{m.fromAddr}</td><td className="sans">{m.subject}</td>
              </tr>
            ))}
            {!mails.length && <tr><td colSpan={4} className="muted sans">no mails</td></tr>}
          </tbody>
        </table>
      )}
    </Flyout>
