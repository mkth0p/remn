...
export function FilterBar({ source, filter, onChange, fields, total, loading, facetsHint, extra }: Props) {
  ...
  const runAi = async () => {
    ...
    try {
      ...
      const f = q.filter
      onChange({
        conditions: (f.conditions ?? []).filter((c) => c.field && c.op),
        logic: f.logic,
        timeRange: f.timeRange,
        hourRange: f.hourRange,
        regex: f.regex?.pattern ? f.regex : undefined,
    ...
    <div className="querybar">
      <div className="row wrap" style={{ gap: 6 }}>
        <div className="search">
          ...
          <input
            data-query-search
            placeholder={source === 'events' ? 'search summary, users, IPs, command lines, raw…  (Enter)' : 'search subject, sender, recipients, body…  (Enter)'}
            value={text}
        ...
        <button className={cls(hasTime)} onClick={() => setShowTime(!showTime)} title="time range (UTC)">
          <IconClock /> {hasTime ? `${shortTs(filter.timeRange?.from)} → ${shortTs(filter.timeRange?.to)}` : 'any time'}
        </button>
        <button className={cls(conds.length)} onClick={() => setShowAdd(true)} title="add a field condition">
          <IconFilter /> {conds.length ? `${conds.length} condition${conds.length === 1 ? '' : 's'}` : 'condition'}
        </button>
        <button
          className={cls(filter.hourRange)}
          onClick={() =>
            onChange({
              ...filter,
              hourRange: filter.hourRange
                ? undefined
                : { from: kase?.settings.businessHours.start ?? 8, to: kase?.settings.businessHours.end ?? 19, outside: true, tz: kase?.settings.businessHours.tz },
            })
          }
          title="only rows outside business hours (Settings)"
        >
          outside hours
        </button>
        <button className={cls(filter.regex?.pattern)} onClick={() => setShowRegex(!showRegex)} title="regular expression on a field">
          regex
        </button>
        <button className={cls(ai)} onClick={() => setAi(!ai)} title="describe what you want in plain language; the local model builds the filter">
          <IconAi /> ask
        </button>
        {saved.length > 0 && (
        ...
        {hasAny ? (
          <>
            <button className="btn icon ghost sm" onClick={() => setSaveName('')} title="save this search">
              <IconSave />
            </button>
            <button className="btn ghost sm" onClick={() => onChange({ sort: filter.sort })} title="clear every filter">
              clear
            </button>
          </>
        ) : null}
        ...
        {extra}
        <span className="mono small dim nowrap" style={{ minWidth: 80, textAlign: 'right' }}>
          {loading ? <Spinner /> : total != null ? `${total.toLocaleString('en-US')} match${total === 1 ? '' : 'es'}` : ''}
        </span>
      </div>
      {showTime && (
        <div className="row wrap" style={{ gap: 6 }}>
          <input
            ...
            value={toLocalInput(filter.timeRange?.from)}
            onChange={(e) => onChange({ ...filter, timeRange: { ...filter.timeRange, from: fromLocalInput(e.target.value) } })}
            title="from (UTC)"
          />
          ...
          <input
            type="datetime-local"
            className="input mono"
            value={toLocalInput(filter.timeRange?.to)}
            onChange={(e) => onChange({ ...filter, timeRange: { ...filter.timeRange, to: fromLocalInput(e.target.value) } })}
            title="to (UTC)"
          />
      ...
      {showRegex && (
        <div className="row wrap" style={{ gap: 6 }}>
          ...
          <input
            ...
            placeholder="JavaScript regular expression, case-insensitive (Enter)"
            value={regex}
            onChange={(e) => setRegex(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && applyRegex()}
            onBlur={applyRegex}
          />
        </div>
      )}
      {ai && (
        <div className="row" style={{ gap: 8 }}>
          <input
            className="input"
            style={{ flex: 1 }}
            placeholder={source === 'events' ? 'e.g. failed logons on the admin account last night from outside the LAN' : 'e.g. mails with macro attachments from lookalike domains this week'}
            value={aiQ}
            onChange={(e) => setAiQ(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && runAi()}
            autoFocus
          />
          <button className="btn primary sm" disabled={aiBusy} onClick={runAi}>
            {aiBusy ? <Spinner /> : 'build filter'}
          </button>
          {aiExpl && (
            <span className="small dim" style={{ maxWidth: 480 }}>
              {aiExpl}
            </span>
          )}
        </div>
      ...
      {hasAny ? (
        <div className="row wrap" style={{ gap: 6 }}>
          ...
          {filter.hourRange && (
            <Chip onRemove={() => onChange({ ...filter, hourRange: undefined })}>
              {filter.hourRange.outside ? 'outside' : 'within'} {filter.hourRange.from}h–{filter.hourRange.to}h
            </Chip>
          )}
          {filter.regex?.pattern && (
            <Chip onRemove={() => onChange({ ...filter, regex: undefined })}>
              /{filter.regex.pattern}/ on {filter.regex.field || '*'}
            </Chip>
          )}
                ...
                onClick={() => {
                  ...
                  if (cond.op === 'in' || cond.op === 'nin')
                    v = String(v)
                      ...
                      .map((s) => s.trim())
                      .filter(Boolean)
                  else if (typeof v === 'string' && /^-?\d+$/.test(v) && ['eq', 'ne', 'gt', 'gte', 'lt', 'lte'].includes(cond.op)) v = Number(v)
                  if (cond.op === 'exists' || cond.op === 'empty') v = undefined
                  setConds([...conds, { ...cond, value: v }])
                  setShowAdd(false)
                }}
              >
                add
        ...
        >
          <div className="row">
            ...
            <input
              className="input mono"
              style={{ flex: 1 }}
              placeholder={cond.op === 'in' || cond.op === 'nin' ? 'comma-separated values' : 'value'}
              value={String(cond.value ?? '')}
              onChange={(e) => setCond({ ...cond, value: e.target.value })}
              disabled={cond.op === 'exists' || cond.op === 'empty'}
              autoFocus={false}
              onKeyDown={(e) => {
                if (e.key === 'Enter') (e.currentTarget.closest('.modal')?.querySelector('.btn.primary') as HTMLButtonElement | null)?.click()
              }}
            />
          </div>
