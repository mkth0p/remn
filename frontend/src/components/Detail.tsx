  ...
  return (
    <div className="drawer">
      <div className="drawer-h">
        <div style={{ flex: 1, minWidth: 0 }} className="ellipsis">
          {title}
        </div>
        {actions}
  ...
  return (
    ...
    >
      <div className="card glow">
        <div className="mono" style={{ color: 'var(--fg-1)' }}>
          {row.summary}
        </div>
        {note && (
      ...
      {tab === 'raw' && (
        <div>
          <div className="row" style={{ justifyContent: 'flex-end' }}>
            <CopyButton text={typeof raw === 'string' ? raw : JSON.stringify(raw, null, 2)} label="copy" />
          </div>
...
export function MailDetail({
  ...
  useEffect(() => {
    ...
    const conditions: Condition[] = rcpts.flatMap(
      (a) =>
        [
          { field: 'targetUser', op: 'eq', value: localPart(a) },
          { field: 'subjectUser', op: 'eq', value: localPart(a) },
          { field: 'upn', op: 'eq', value: a },
        ] as Condition[],
    )
    const f: Filter = {
      conditions,
      logic: 'or',
      timeRange: { from: new Date(row.date - 15 * 60_000).toISOString(), to: new Date(row.date + 72 * 3_600_000).toISOString() },
  ...
  const authBadge = (k: string) => {
    ...
    return (
      <Badge key={k} sev={sev} title={String((auth?.raw as string[] | undefined)?.join('\n') ?? '')}>
        {k} {v}
      </Badge>
    )
  }
  const explain = () => {
    setAiPrompt(
      `Analyse this e-mail (mail id ${row.id}) for phishing / BEC indicators and tell me what to check next. Use get_mail with includeBody if needed. Summary: subject "${row.subject}", from ${row.fromName} <${row.fromAddr}>, origin IP ${row.originIp}, risk ${row.risk}, flags: ${row.flags.join(', ')}`,
    )
    setView('ai')
  }
    ...
    <div className="col" style={{ gap: 14 }}>
      <div className="section">
        ...
        <div className="kv">
          ...
          <div className="v click" onClick={() => setEntity({ kind: 'address', value: row.fromAddr })} title="open the sender page">
            {row.fromAddr}
          </div>
          {senderDomain && (
            <>
              <div className="k">domain</div>
              <div className="v click" onClick={() => setEntity({ kind: 'domain', value: senderDomain })} title="open the domain page">
                {senderDomain}
              </div>
            </>
          )}
          {row.replyTo
            .filter((r) => r.addr && r.addr.toLowerCase() !== row.fromAddr.toLowerCase())
            .map((r) => (
              <Fragment key={r.addr}>
                <div className="k" style={{ color: 'var(--warn)' }}>
                  reply-to
                </div>
                <div className="v click" onClick={() => setEntity({ kind: 'address', value: r.addr })}>
                  {r.addr}
                </div>
              </Fragment>
            ))}
          {row.to.slice(0, 4).map((t, i) => (
            <Fragment key={'to' + i}>
              <div className="k">{i === 0 ? 'recipient' : ''}</div>
              <div className="v click" onClick={() => setEntity({ kind: 'user', value: t.addr || t.name })} title="open the user page">
                {t.addr || t.name}
              </div>
            </Fragment>
          ))}
          {row.to.length > 4 && (
            <>
              <div className="k" />
              <div className="v muted">+{row.to.length - 4} more</div>
            </>
          )}
          {row.originIp && (
            <>
              <div className="k">origin ip</div>
              <div className="v click" onClick={() => setEntity({ kind: 'ip', value: String(row.originIp) })} title="open the IP page">
                {row.originIp}
              </div>
            </>
          )}
      ...
      <div className="section">
        ...
        {row.senderPrevalence ? (
          <div className="kv">
            <div className="k">prevalence</div>
            <div className="v">
              <Badge sev={row.senderPrevalence === 'new' ? 'high' : row.senderPrevalence === 'rare' ? 'medium' : 'ok'}>{row.senderPrevalence}</Badge>
            </div>
            <div className="k">prior mails</div>
            <div className="v">
              {row.senderPriorCount ?? 0}
              {row.senderDaysKnown != null ? ` · known ${row.senderDaysKnown} day(s)` : ''}
            </div>
            <div className="k">first seen</div>
            <div className="v">{row.senderFirstSeen ? fmtTs(row.senderFirstSeen) : '—'}</div>
            <div className="k">solicited</div>
            <div className="v">
              {row.senderSolicited === true ? 'yes - the recipient wrote to this sender before' : row.senderSolicited === false ? 'no - first contact' : 'unknown (inbox-only export)'}
            </div>
            {row.senderAuthRegression && (
              <>
                <div className="k">auth</div>
                <div className="v" style={{ color: 'var(--danger)' }}>
                  authentication regressed vs earlier mails from this sender
                </div>
              </>
            )}
            {row.campaignId && (
              <>
                ...
                <div className="v click" onClick={() => pivot(row.campaignId!, 'campaignId', 'mails')}>
                  {row.campaignSize ?? '?'} mail(s) · {row.campaignSenders ?? '?'} sender(s)
                </div>
              </>
            )}
          </div>
      ...
      <div className="section">
        ...
        {findings.slice(0, 8).map((f) => (
          <div key={f.id} className="row click" style={{ gap: 8, cursor: 'pointer' }} onClick={() => setView('findings')} title={f.description}>
            ...
            <Badge className="small">{f.status}</Badge>
          </div>
        ))}
      </div>
      <div className="section">
        ...
        {row.assessment ? (
          <div className="small dim">
            <div>
              Attack evidence <strong>{row.assessment.confidence}</strong>
              {row.assessment.expectedSender ? ' · expected correspondent' : ''}
            </div>
            <div>
              {Object.entries(row.assessment.groups)
                .filter(([, w]) => w > 0)
                .map(([g, w]) => `${g} (${w})`)
                .join(' · ') || 'no signal family'}{' '}
              · attachments {row.assessment.attachmentRisk}
            </div>
            {row.assessment.limitations.map((s) => (
              <div key={s} style={{ color: 'var(--warn)' }}>
                {s}
              </div>
            ))}
          ...
          <div className="small dim">
            legacy score - use "rescore" to apply the current calibration. Drivers:{' '}
            {row.flags
        ...
        {row.reputation?.worst && (
          <div className="small">
            reputation <Badge sev={row.reputation.worst === 'malicious' ? 'critical' : 'medium'}>{row.reputation.worst}</Badge>
          </div>
        )}
      </div>
      <div className="section">
        ...
        <div className="kv">
          ...
          <div className="v">
            {(row.sourceName ?? '').replace(/^.*[\\/]/, '') || '—'} <span className="muted">({row.sourceFormat})</span>
          </div>
          ...
          {row.xMailer && (
            <>
              ...
              <div className="v">{row.xMailer}</div>
            </>
          )}
          <div className="k">row</div>
          <div className="v">#{row.id}</div>
        </div>
      </div>
    </div>
  )
  ...
  const head = (
    <div className="col" style={{ gap: 4, minWidth: 0 }}>
      <div className="row" style={{ gap: 8, alignItems: 'center', minWidth: 0 }}>
        <Risk value={row.risk} />
        <div className="ellipsis" style={{ fontWeight: 600, color: 'var(--fg-1)', fontSize: 13.5, flex: 1, minWidth: 0 }} title={row.subject}>
          {row.subject || '(no subject)'}
        </div>
        {worstFinding && (
          <Sev sev={worstFinding}>
            {activeFindings.length} finding{activeFindings.length === 1 ? '' : 's'}
          </Sev>
        )}
        <AddToTimeline
          ts={row.date}
          text={`Mail "${row.subject || '(no subject)'}" from ${row.fromAddr} to ${row.to
            .map((t) => t.addr)
            .slice(0, 2)
            .join(', ')}`}
        ...
        {onPaneMax && (
          <button className="btn icon ghost sm" onClick={() => onPaneMax(!paneMax)} title={paneMax ? 'restore the table' : 'expand the message'}>
            <IconChevronDown style={{ transform: paneMax ? 'none' : 'rotate(180deg)' }} />
          </button>
        )}
        <button className="btn icon ghost sm" onClick={onClose} title="close (Esc)">
          <IconClose />
        </button>
      </div>
      <div
        ...
        title={`${row.fromName ? `"${row.fromName}" ` : ''}<${row.fromAddr}> → ${row.to.map((t) => t.addr || t.name).join(', ') || '(undisclosed)'}${row.cc.length ? ` · cc ${row.cc.map((t) => t.addr).join(', ')}` : ''} · ${fmtTs(row.date)}`}
      >
        <span className="click" onClick={() => setEntity({ kind: 'address', value: row.fromAddr })}>
          {row.fromName ? `"${row.fromName}" ` : ''}&lt;{row.fromAddr}&gt;
        </span>
        {' → '}
        {row.to
          .map((t) => t.addr || t.name)
          .slice(0, 3)
          .join(', ') || '(undisclosed)'}
        {row.to.length > 3 ? ` +${row.to.length - 3}` : ''}
        {row.cc.length ? <span className="muted"> · cc {row.cc.length}</span> : null}
        {' · '}
        {fmtTs(row.date)}
      </div>
      <div className="row wrap" style={{ gap: 4 }}>
        {['spf', 'dkim', 'dmarc'].map(authBadge)}
        {auth?.compauth ? authBadge('compauth') : null}
        {auth?.arc ? authBadge('arc') : null}
        {row.replyTo.some((r) => r.addr && r.addr.toLowerCase() !== row.fromAddr.toLowerCase()) && (
          <Badge sev="medium" title={row.replyTo.map((r) => r.addr).join(', ')}>
            reply-to differs
          </Badge>
        )}
        {lookalike.length > 0 && (
          <Badge sev="critical" title={lookalike.map((m) => `${m.reference} (${m.method}, ${m.kind})`).join('; ')}>
            lookalike of {lookalike[0].reference}
          </Badge>
        )}
        {shownFlags.map((f) => (
          <Flag key={f} name={f} />
        ...
        {hiddenFlags > 0 && (
          <button className="btn link small" onClick={() => setAllFlags(true)} title="show every flag, including the quiet observations">
            +{hiddenFlags} more
          </button>
        )}
        {allFlags && visibleFlags.length > 8 && (
          <button className="btn link small" onClick={() => setAllFlags(false)}>
            fewer
          </button>
        )}
      </div>
    ...
    <Tabs
      tabs={[
        ...
        { id: 'headers' as MailTab, label: 'Headers' },
        {
          ...
          label: (
            <span>
              Hops <span className="n">{row.hopCount}</span>
            </span>
          ),
        },
        {
          ...
          label: (
            <span>
              URLs <span className="n">{row.urlCount}</span>
            </span>
          ),
        },
        {
          id: 'attachments' as MailTab,
          label: (
            <span>
              Attachments <span className="n">{row.attachmentCount}</span>
            </span>
          ),
        },
    ...
    <>
            ...
            <div className="row wrap" style={{ gap: 8 }}>
              {Object.keys(row.keywordHits ?? {}).length > 0 && (
                <span className="small dim">
                  lexicon:{' '}
                  {Object.entries(row.keywordHits)
                    .map(([k, v]) => `${k} → ${v.slice(0, 4).join(', ')}`)
                    .join(' · ')}
                </span>
              )}
              {body?.visibleText && body.bodyText && body.visibleText.length < body.bodyText.length * 0.6 && (
                <Badge sev="medium" title="a large part of the text is not visible when rendered">
                  hidden text
                </Badge>
              )}
            </div>
          )}
          <pre className="codeblock" style={{ fontFamily: 'var(--sans)', fontSize: 12.5, lineHeight: 1.5 }}>
            {body?.bodyText || body?.visibleText || row.textPreview || '(empty)'}
          </pre>
        </div>
      )}
      {tab === 'html' &&
        (body?.bodyHtml ? (
          <iframe className="mailframe" sandbox="" srcDoc={srcdoc} title="mail html (sandboxed: images blocked, links disabled, no scripts)" />
        ) : (
          <div className="muted">no HTML body{body ? '' : ' (loading…)'}</div>
        ))}
        ...
        <table className="table compact">
          ...
          <tbody>
            {(row.hops as { index: number; from?: string; fromIp?: string; by?: string; with?: string; ts?: number; delayS?: number | null }[]).map((h) => (
              <tr key={h.index}>
                <td>{h.index}</td>
                <td className="ellipsis" style={{ maxWidth: 220 }} title={h.from}>
                  {h.from}
                </td>
                <td className="click" onClick={() => h.fromIp && setEntity({ kind: 'ip', value: h.fromIp })}>
                  {h.fromIp}
                </td>
      ...
      {tab === 'urls' && (
        <table className="table compact">
          ...
          <tbody>
            {row.urls.map((u, i) => (
              <tr key={i}>
                <td className="click" style={{ maxWidth: 420, wordBreak: 'break-all' }} onClick={() => setEntity({ kind: 'domain', value: u.domain || u.host })} title="open the domain page">
                  {u.defanged}
                </td>
                <td className="ellipsis sans" style={{ maxWidth: 160 }}>
                  {u.text}
                </td>
                <td>
                  <div className="row wrap" style={{ gap: 3 }}>
                    {u.flags.map((f) => (
                      <Flag key={f} name={'url_' + f} />
                    ))}
                  </div>
                </td>
              </tr>
            ...
            {!row.urls.length && (
              <tr>
                <td colSpan={3} className="muted sans">
                  no URLs
                </td>
              </tr>
            )}
      ...
      {tab === 'related' && (
        <div className="col" style={{ gap: 12 }}>
          ...
          <div className="section">
            <h3>
              Recipient activity after delivery <span className="muted">(15 min before to 72 h after, first 300)</span>
            </h3>
            {related.busy && <div className="muted small">loading…</div>}
            {!related.busy && related.loaded && !related.events.length && (
              <div className="muted small">
                no host or cloud events for{' '}
                {row.to
                  .slice(0, 3)
                  .map((t) => localPart(t.addr || t.name))
                  .join(', ') || 'the recipients'}{' '}
                in that window
              </div>
            )}
                  ...
                  {related.events.map((e) => (
                    ...
                    >
                      ...
                      <td className="muted nowrap">
                        {e.ts && row.date
                          ? (() => {
                              const d = Math.round((e.ts - row.date) / 60_000)
                              return `${d >= 0 ? '+' : ''}${d}m`
                            })()
                          : ''}
                      </td>
  ...
  return (
    <Drawer
      title={
        <span className="mono">
          <Risk value={row.risk} /> {row.subject || '(no subject)'}
        </span>
      }
      onClose={onClose}
      actions={
