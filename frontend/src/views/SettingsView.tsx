...
export function SettingsView() {
  ...
  useEffect(() => {
    if (ds?.kind === 'server') ds.summary().then((s) => setStoreSize(((s as { sizeBytes?: number }).sizeBytes ?? null))).catch(() => setStoreSize(null))
  ...
  const migrate = async () => {
    ...
    if (!confirm(`Move this case to the server store (DuckDB on this machine)? ${total.toLocaleString('en-US')} rows will be transferred and removed from the browser. Findings will be cleared (re-run the rules afterwards).`)) return
  ...
  return (
    <div className="view">
      <div className="view-header"><h1>Settings</h1><span className="sub">case-level context used by the rules and the AI</span></div>
        ...
        <div className="grid-2">
          <div className="panel">
            ...
            <div className="panel-b col">
              ...
              <label className="field"><span>analyst</span><input className="input" value={analyst} onChange={(e) => setAnalyst(e.target.value)} placeholder="recorded in the chain of custody" /></label>
              <label className="field"><span>notes</span><textarea className="textarea" value={notes} onChange={(e) => setNotes(e.target.value)} /></label>
              <div className="row"><button className="btn primary sm" onClick={saveMeta}>save</button></div>
            </div>
          </div>
          <div className="panel">
            <div className="panel-h">storage</div>
            <div className="panel-b col">
              <div className="row"><Badge sev={isServer ? 'accent' : 'info'}>{isServer ? 'server store (DuckDB)' : 'browser store (IndexedDB)'}</Badge>{isServer && <span className="mono small dim">{kase.serverKey} · {storeSize != null ? fmtBytes(storeSize) : '…'}</span>}</div>
              <div className="hint">{isServer ? 'Rows live in a DuckDB file under the server data folder on this machine; the browser keeps cases, findings, notes and AI sessions. Suited to gigabytes of logs and mailboxes.' : 'Rows live in this browser only (portable, zero server state). Suited to cases under a few hundred MB.'}</div>
              {!isServer && (
                <div className="col">
                  <button className="btn primary sm" onClick={migrate} disabled={!!migrating}>convert this case to the server store</button>
                  {migrating && <><Progress indeterminate /><div className="small dim mono">{migrating}</div></>}
                </div>
              )}
              <label className="field"><span>suggest the server store for files larger than (MB)</span><input type="number" className="input mono" min={10} value={threshold} onChange={(e) => { const v = Number(e.target.value) || 150; setThreshold(v); getDb().kv.put({ key: 'storeThresholdMb', value: v }) }} /></label>
              {isServer && (
                <>
                  <Toggle on={s.deepAttachments !== false} onChange={(v) => patch({ deepAttachments: v })} label="deep attachment analysis at ingestion (macros, PDF, archives) - slower on huge mailboxes" />
            ...
            <div className="panel-b col">
              ...
              {aiCfg.transport === 'browser' && (
                <label className="field"><span>local Ollama URL (as seen from this browser)</span>
                  <div className="row">
                    <input className="input mono" style={{ flex: 1 }} value={urlDraft} onChange={(e) => setUrlDraft(e.target.value)} onBlur={() => saveAi({ ollamaUrl: urlDraft.trim().replace(/\/+$/, '') || 'http://localhost:11434' })} placeholder="http://localhost:11434" />
                  </div>
                </label>
              )}
          ...
          <div className="panel">
            ...
            <div className="panel-b col">
              <div className="row">
                <label className="field"><span>business hours start</span><input type="number" className="input mono" min={0} max={23} value={s.businessHours.start} onChange={(e) => patch({ businessHours: { ...s.businessHours, start: Number(e.target.value) } })} /></label>
                <label className="field"><span>end</span><input type="number" className="input mono" min={0} max={24} value={s.businessHours.end} onChange={(e) => patch({ businessHours: { ...s.businessHours, end: Number(e.target.value) } })} /></label>
                <label className="field" style={{ flex: 1 }}><span>time zone (IANA)</span><input className="input mono" value={s.businessHours.tz} onChange={(e) => patch({ businessHours: { ...s.businessHours, tz: e.target.value } })} placeholder="Europe/Paris" /></label>
              </div>
              <label className="field"><span>weekend days</span>
                <div className="row wrap">{['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((d, i) => <label key={d} className="checkbox small"><input type="checkbox" checked={s.weekendDays.includes(i)} onChange={(e) => patch({ weekendDays: e.target.checked ? [...s.weekendDays, i].sort() : s.weekendDays.filter((x) => x !== i) })} /> {d}</label>)}</div>
              </label>
              <Toggle on={local} onChange={(v) => { setLocal(v); setLocalTime(v); getDb().kv.put({ key: 'localTime', value: v }); toast('info', v ? 'timestamps shown in local time' : 'timestamps shown in UTC') }} label="display timestamps in local time instead of UTC (display only - data stays UTC)" />
            </div>
          </div>
          <div className="panel">
              ...
              <div className="col" style={{ gap: 4 }}>
                {providers.map((p) => (
                  <label key={p.name} className="checkbox small" title={p.description}>
                    <input type="checkbox" disabled={!p.configured} checked={!s.providers.length ? p.configured : s.providers.includes(p.name)} onChange={(e) => { const base = s.providers.length ? s.providers : providers.filter((x) => x.configured).map((x) => x.name); patch({ providers: e.target.checked ? Array.from(new Set([...base, p.name])) : base.filter((x) => x !== p.name) }) }} />
                    <span className="mono">{p.name}</span> <Badge sev={p.configured ? 'ok' : 'info'}>{p.configured ? 'configured' : p.needsKey ? `needs ${p.needsKey} key` : 'unavailable'}</Badge> <span className="muted">{p.kinds.join(', ')}</span>
                  </label>
                ))}
              </div>
              <div className="hint">Keys are read by the server from <code>.env</code> (see <code>.env.example</code>). Offline block lists go to <code>backend/data/lists</code>, YARA rules to <code>backend/data/yara</code>, GeoLite2 databases to <code>backend/data/geoip</code>.</div>
            </div>
          </div>
          <div className="panel">
            ...
            <div className="panel-b col">
              ...
              <div className="kv small">
                ...
                <div className="k">ollama</div><div className="v">{health?.ollama.host} · {health?.ollama.reachable ? `${health.ollama.models.length} model(s)` : health?.ollama.error}</div>
                <div className="k">optional</div><div className="v">PST/OST {health?.optional.pst ? 'yes' : 'no (pip install libpff-python)'} · YARA {health?.optional.yara ? `yes (${health.optional.yaraRules} rule file(s))` : 'no'}</div>
              </div>
              <div className="divider" />
              <div className="row"><button className="btn danger sm" onClick={wipe}>delete all case data{isServer ? ' (browser + server store)' : ' from this browser'}</button></div>
            </div>
          </div>
        </div>
      </div>
      {suggestions && (
        <Modal title="Suggested trusted senders" onClose={() => setSuggestions(null)} footer={<button className="btn primary" onClick={applySuggestions} disabled={!picked.size}>add {picked.size} to trusted senders</button>}>
          ...
          <div className="col" style={{ gap: 4, maxHeight: 320, overflowY: 'auto' }}>
            {suggestions.map((f) => (
              <label key={f.registrable} className="checkbox small mono">
                <input type="checkbox" checked={picked.has(f.registrable)} onChange={(e) => { const next = new Set(picked); if (e.target.checked) next.add(f.registrable); else next.delete(f.registrable); setPicked(next) }} />
                <span>{f.registrable}</span> <span className="dim">{f.count} mail(s) · e.g. {f.sample}</span>
              </label>
            ))}
          </div>
        </Modal>
