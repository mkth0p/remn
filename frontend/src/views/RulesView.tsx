  ...
  return (
    <div className="card" style={{ padding: '8px 12px', minWidth: 260, flex: '1 1 260px', outline: selected ? '1px solid var(--accent)' : undefined }} data-pack={pack.id}>
      ...
      <div className="small muted row" style={{ gap: 8, marginTop: 4, flexWrap: 'wrap' }}>
        <a href={safeHref(pack.upstream?.url)} target="_blank" rel="noreferrer noopener">
          {pack.upstream?.repo}
          {sha ? `@${sha}` : ''}
        </a>
        ...
        <span>
          ·{' '}
          {safeHref(pack.license?.url) ? (
            <a href={safeHref(pack.license?.url)} target="_blank" rel="noreferrer noopener">
              {pack.license?.name}
            </a>
          ) : (
            pack.license?.name
        ...
        {pack.licenseText && (
          <a href={`/api/rules/packs/${encodeURIComponent(pack.id)}/license`} target="_blank" rel="noreferrer">
            licence text
          </a>
        )}
        <span className="spacer" />
        {on && (
          <button className={`btn xs${selected ? ' primary' : ''}`} onClick={onShow} disabled={busy}>
            {busy ? <Spinner /> : selected ? 'showing' : 'show rules'}
          </button>
        )}
      </div>
    </div>
    ...
    <div className="view">
      <div className="view-header">
        ...
        {lastRun && lastRun.diagnostics.some((d) => d.reason === 'missing_setting') && (
          <button className="btn sm" onClick={() => useStore.getState().setView('settings')} title="rules referencing empty settings lists are disarmed">
            ⚠ configure Settings to arm {lastRun.diagnostics.filter((d) => d.reason === 'missing_setting').length} rule(s)
          </button>
        )}
        <RuleImport kind="sigma" />
        <RuleImport kind="sublime" />
        <button className="btn sm primary" onClick={() => setEdit({ yaml: TEMPLATE })}>
          <IconPlus /> new rule
        </button>
      </div>
      {packs.length > 0 && (
          ...
          <div className="row" style={{ gap: 10, flexWrap: 'wrap', alignItems: 'stretch' }}>
            {packs.map((p) => (
              <PackCard
                ...
                on={packsOn.has(p.id)}
                busy={loading}
                selected={packFilter === p.id}
                onToggle={(v) => togglePack(p, v)}
                onShow={() => setPackFilter(packFilter === p.id ? '' : p.id)}
              />
            ))}
          </div>
        </div>
      ...
      <div className="row" style={{ padding: '8px 16px', borderBottom: '1px solid var(--line)', gap: 8 }}>
        ...
        <input
          className="input"
          style={{ width: 380 }}
          placeholder="describe a detection and let the local model draft the YAML…"
          value={aiAsk}
          onChange={(e) => setAiAsk(e.target.value)}
          onKeyDown={(e) => e.key === 'Enter' && askAi()}
        />
        <button className="btn sm" onClick={askAi} disabled={aiBusy}>
          {aiBusy ? <Spinner /> : <IconAi />} draft
        </button>
      </div>
      <div className="view-body">
            ...
            {shown.slice(0, MAX_ROWS).map((r) => (
              <tr key={r.file + r.rule.id}>
                ...
                <td>
                  ...
                  <div className="small muted">{r.rule.description}</div>
                </td>
                <td>{r.rule.source}</td>
                <td>
                  <Badge sev={r.rule.severity}>{r.rule.severity}</Badge>
                </td>
                <td className="small">
                  {r.rule.group_by?.length
                    ? `group ${r.rule.group_by.join('+')}${r.rule.window ? ` / ${r.rule.window}` : ''}${r.rule.threshold ? ` ${r.rule.threshold}` : ''}`
                    : r.rule.time
                      ? 'temporal'
                      : 'match'}
                  {r.rule.then ? ' → follow-up' : ''}
                </td>
                <td>
                  {(() => {
                    ...
                    if (d)
                      return (
                        <Badge sev={DIAG_LABEL[d.reason].sev} title={d.detail || `${d.matched} matched · ${d.afterExclude} after exclude · ${d.afterTime} after time filter`}>
                          {DIAG_LABEL[d.reason].label}
                        </Badge>
                      )
                ...
                <td>
                  <Badge sev={r.origin === 'custom' ? 'accent' : 'info'} title={r.file}>
                    {originLabel(r)}
                  </Badge>
                </td>
                <td className="row">
                  <button
                    className="btn xs"
                    onClick={() => setEdit({ id: r.origin === 'custom' ? Number(r.file.split(':')[1]) : undefined, yaml: yamlOf(r) })}
                    title={r.origin === 'custom' ? 'edit' : 'copy as custom rule'}
                  >
                    <IconEdit />
                  </button>
                  {r.origin === 'custom' && (
                    <button className="btn xs danger" onClick={() => remove(r)}>
                      <IconTrash />
                    </button>
                  )}
                </td>
        ...
        {shown.length > MAX_ROWS && (
          <div className="hint" style={{ padding: 12 }}>
            showing the first {MAX_ROWS} of {fmtNum(shown.length)} rules — narrow with the search box, the source or the origin filter.
          </div>
        )}
      ...
      {edit && (
        <Modal
          title={edit.id ? 'Edit custom rule' : 'New custom rule'}
        ...
        >
          ...
          <div className="hint">
            Operators: field|eq (default), ne, in, nin, contains, not_contains, contains_any, contains_all, startswith, not_startswith, endswith, not_endswith, re, not_re, gt, gte, lt, lte, exists,
            empty, in_setting, nin_setting, levenshtein, length ("&lt; 500"), contains_cs / startswith_cs / endswith_cs (case-sensitive). Groups: any_of / all_of / not. Aggregation: group_by, window,
            threshold, distinct, then, time, exclude. Custom rules with the same id override bundled and pack rules.
          </div>
        </Modal>
      )}
    </div>
