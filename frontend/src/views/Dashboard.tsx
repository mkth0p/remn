  ...
  return (
    <div className="view">
      <div className="view-header">
        <h1>Dashboard</h1>
        <span className="sub">{kase.name} · created {fmtTs(kase.createdAt, { date: true })}{kase.analyst ? ` · ${kase.analyst}` : ''}</span>
        <Badge sev={isServer ? 'accent' : 'info'} title={isServer ? `DuckDB case store ${kase.serverKey}` : 'rows stored in this browser (IndexedDB)'}>{isServer ? 'server store' : 'browser store'}</Badge>
      </div>
      <div className="view-body col" style={{ gap: 14 }}>
        ...
        <div className="grid-2">
          <div className="panel">
            ...
            <div className="panel-b col">
              ...
              <Jobs />
              <div className="hint">{isServer ? 'Files are hashed in the browser, uploaded in chunks to the local server and parsed into a DuckDB case store on this machine. Nothing leaves the host.' : 'Files are hashed (SHA-256) in the browser, parsed by the local server and stored only in this browser\'s IndexedDB. The server keeps nothing.'}</div>
            </div>
          </div>
          <div className="panel">
            <div className="panel-h">top findings <span className="spacer" /><button className="btn xs ghost" onClick={() => setView('findings')}>all →</button></div>
            <div className="panel-b col" style={{ gap: 6 }}>
              ...
              {findings.map((f) => (
                <div key={f.id} className="row small" style={{ gap: 8 }}>
                  ...
                  <span className="ellipsis" style={{ flex: 2, minWidth: 0 }} title={f.title}>{f.title}</span>
                  <span className="mono dim ellipsis" style={{ flex: 1, minWidth: 0 }}>{Object.values(f.entities).slice(0, 2).join(' · ')}</span>
        ...
        <div className="grid-3">
          <div className="card">
            <div className="stat"><span className="label">{isServer ? 'server store size' : 'browser storage'}</span><span className="value" style={{ fontSize: 16 }}>{isServer ? fmtBytes(summary?.sizeBytes ?? 0) : storage ? `${fmtBytes(storage.usage)} / ${fmtBytes(storage.quota)}` : '—'}</span></div>
            {!isServer && storage && <div className="progress" style={{ marginTop: 6 }}><div style={{ width: `${Math.min(100, (storage.usage / Math.max(1, storage.quota)) * 100)}%` }} /></div>}
            {isServer && <div className="small muted">DuckDB file on this machine · {kase.serverKey?.slice(0, 8)}…</div>}
          </div>
          <div className="card">
            <div className="stat"><span className="label">local AI</span><span className="value" style={{ fontSize: 16 }}>{aiStatus.reachable ? (aiCfg.model || health?.ollama.defaultModel || 'ready') : aiStatus.reachable === null ? 'checking…' : 'Ollama unreachable'}</span></div>
            <div className="small muted">{aiCfg.transport === 'browser' ? `browser-direct · ${aiCfg.ollamaUrl}` : 'via REMN server'}{aiStatus.reachable ? ` · ${aiStatus.models ?? 0} model(s)` : aiStatus.error ? ` · ${aiStatus.error.slice(0, 120)}` : ''}</div>
          </div>
          <div className="card">
            <div className="stat"><span className="label">external lookups</span><span className="value" style={{ fontSize: 16, color: kase.settings.networkAllowed ? 'var(--warn)' : 'var(--ok)' }}>{kase.settings.networkAllowed ? 'ENABLED' : 'disabled'}</span></div>
            <div className="small muted">{health?.providers.filter((p) => p.configured).map((p) => p.name).join(', ') || 'no provider configured'}</div>
          </div>
        </div>
      </div>
    </div>
