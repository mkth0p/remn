...
export function AiView() {
  ...
  useEffect(() => {
    ...
    if (!m) { setCanTools(true); return }
    getTransport()
      .capabilities(m)
      .then((caps) => { if (!dead) setCanTools(!caps.length || caps.includes('tools')) })
      .catch(() => { if (!dead) setCanTools(true) })
    return () => { dead = true }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    <div className="view">
      <div className="split" style={{ gridTemplateColumns: '240px 1fr' }}>
        <div className="left col" style={{ padding: 8, gap: 6 }}>
          <button className="btn sm" onClick={newSession}><IconPlus /> new session</button>
          {sessions.map((s) => (
          ...
          <div className="chat-log" ref={logRef}>
            {!messages.length && (
              <div className="col" style={{ maxWidth: 760 }}>
                ...
                {STARTERS.map((s) => <button key={s} className="btn ghost sm" style={{ justifyContent: 'flex-start', whiteSpace: 'normal', textAlign: 'left' }} onClick={() => send(s)}>› {s}</button>)}
              </div>
            )}
            {messages.map((m, i) => {
              if (m.role === 'tool') return showTools ? <div key={i} className="msg tool"><div className="role">tool result · {m.tool_name}</div>{m.content.length > 1200 ? m.content.slice(0, 1200) + `… (${m.content.length} chars)` : m.content}</div> : null
              if (m.role === 'assistant') {
                return (
                  <div key={i} className="col" style={{ gap: 6, alignSelf: 'flex-start', maxWidth: '100%' }}>
                    {m.thinking && showTools && <div className="msg thinking"><div className="role">thinking</div>{m.thinking.slice(0, 2000)}</div>}
                    {m.tool_calls?.length ? <div className="msg tool"><div className="role">tool calls</div>{m.tool_calls.map((c, j) => <div key={j}>→ <b>{c.name}</b>({JSON.stringify(c.arguments)})</div>)}</div> : null}
                    {m.content && <div className="msg assistant md"><div className="role">remn · {m.stats?.eval_count ? `${m.stats.eval_count} tokens` : ''}</div><div dangerouslySetInnerHTML={{ __html: renderMarkdown(m.content) }} /></div>}
                  </div>
                )
              }
              return <div key={i} className="msg user"><div className="role">analyst</div>{m.content}</div>
            })}
            {busy && (liveThinking || live) && (
              <div className="col" style={{ gap: 6, alignSelf: 'flex-start', maxWidth: '100%' }}>
                {liveThinking && showTools && <div className="msg thinking"><div className="role">thinking…</div>{liveThinking.slice(-1500)}</div>}
                {live && <div className="msg assistant md"><div className="role">remn</div><div dangerouslySetInnerHTML={{ __html: renderMarkdown(live) }} /></div>}
              </div>
            )}
            {busy && !live && !liveThinking && <div className="msg assistant"><Spinner /> <span className="dim small">working…</span></div>}
          </div>
          <div className="chat-input">
            <textarea className="textarea mono" value={input} onChange={(e) => setInput(e.target.value)} placeholder="ask about the evidence… (Enter to send, Shift+Enter for a new line)" onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send() } }} />
