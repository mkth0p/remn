...
const FindingSeverity = ({ finding }: { finding: Finding }) => (
  <span title={finding.severityOverride ? `Review severity override: ${finding.severityOverride}; rule: ${finding.severity}` : undefined}>
    <Sev sev={effectiveSeverity(finding)}>
      {effectiveSeverity(finding)}
  ...
  return (
    <div className="section">
      ...
      <p className="small muted">
        A saved review overrides the rule severity for the findings marked with *. Rule refreshes preserve these overrides. Resetting keeps review status and notes.
        {chain ? ' Chain severity is managed separately on the Review page.' : ''}
      </p>
      <button className="btn sm" disabled={busy} onClick={onReset}>
        {busy ? 'resetting…' : chain ? 'reset finding overrides' : 'reset to rule severity'}
      </button>
    </div>
  )
}
...
export function FindingsView() {
  ...
  const openEntity = (k: string, v: string, source: 'events' | 'mails') => {
    const kind = entityKind(k)
    if (kind && v && !v.includes(',')) setEntity({ kind, value: v })
    else pivot(v, k, source)
  }
  ...
  const reload = useCallback(() => {
    ...
    db.findings
      ...
      .equals(kase.id)
      .toArray()
      .then((f) => setAll(f.sort((a, b) => ORDER.indexOf(effectiveSeverity(a)) - ORDER.indexOf(effectiveSeverity(b)) || (b.ts ?? 0) - (a.ts ?? 0))))
  ...
  const resetSeverityFor = async (findings: Finding[]) => {
    ...
    try {
      await resetFindingSeverityOverrides(
        kase.id!,
        findings.filter((f) => f.severityOverride).map((f) => f.id!),
      )
  ...
  const explain = (f: Finding) => {
    setAiPrompt(
      `Explain this finding and propose the next investigation steps. Rule ${f.ruleId} (${f.severity}): ${f.title}. ${f.description ?? ''} Entities: ${JSON.stringify(f.entities)}. ${f.count} matching row(s) in ${f.source}, first ${f.ts ? new Date(f.ts).toISOString() : 'n/a'}. Use the tools to look at the referenced rows (ids ${f.refs.slice(0, 20).join(', ')}).`,
    )
    setView('ai')
  }
  const explainIncident = (i: Incident) => {
    setAiPrompt(
      `Assess this incident and propose the next investigation steps. ${i.kind === 'mail' ? 'Mail' : 'Entity'} "${i.title}" (${i.severity}), ${i.findings.length} finding(s) from ${i.rules.length} rule(s), ${i.ts ? new Date(i.ts).toISOString() : 'n/a'} to ${i.tsEnd ? new Date(i.tsEnd).toISOString() : 'n/a'}. Entities: ${JSON.stringify(i.entities)}. Findings: ${i.findings
        .slice(0, 20)
        .map((f) => `${f.severity} ${f.ruleId}: ${f.title}`)
        .join(' | ')}. Referenced rows (${i.source}): ${i.refs.slice(0, 20).join(', ')}.`,
    )
    setView('ai')
  }
  ...
  const columns: Column<Finding>[] = [
    { key: 'severity', label: 'severity', width: 104, render: (r) => <FindingSeverity finding={r} /> },
    {
      ...
      render: (r) => (
        <span className="sans ellipsis" title={r.description}>
          {r.title}
          {r.escalation ? <span className="muted"> · {r.escalation}</span> : null}
          {r.id != null && membership.has(r.id) && r.ruleId !== 'chain' ? (
            <>
              {' '}
              <Badge sev="outline" title="its rows are steps of an attack chain: decided with the chain on the Review page">
                chain
              </Badge>
            </>
          ) : null}
          {r.chainUnlinked ? (
            <>
              {' '}
              <Badge sev="outline" title="taken out of its attack chain: decided on its own">
                unlinked
              </Badge>
            </>
          ) : null}
    ...
    },
    {
      ...
      render: (r) => (
        <span className="row" style={{ gap: 4 }}>
          {r.attack.slice(0, 2).map((t) => (
            <Badge key={t} sev="outline">
              {t}
            </Badge>
          ))}
          {r.attack.length > 2 ? <span className="muted">+{r.attack.length - 2}</span> : null}
        </span>
      ),
    },
  ...
  const incidentColumns: Column<Incident>[] = [
    {
      ...
      width: 104,
      render: (r) => (
        <span title={r.kind !== 'chain' && r.findings.some((f) => f.severityOverride) ? 'Includes review severity overrides; open the incident to see the rule severities or reset.' : undefined}>
          <Sev sev={r.severity}>
            {r.severity}
            {r.kind !== 'chain' && r.findings.some((f) => f.severityOverride) ? '*' : ''}
          </Sev>
        </span>
      ),
    },
    {
      key: 'title',
      label: 'incident',
      width: 'minmax(300px, 1.8fr)',
      render: (r) => (
        <span className="sans ellipsis" title={r.subtitle}>
    ...
    { key: 'kind', label: 'kind', width: 74, render: (r) => KIND_LABEL[r.kind] },
    {
      ...
      render: (r) => (
        <span className="row" style={{ gap: 6 }}>
          <span className="mono">{fmtNum(r.findings.length)}</span>
          <span style={{ width: 70 }}>
            <SevBar counts={sevCounts(r.findings)} />
          </span>
        </span>
      ),
    },
  ...
  const memberRow = (f: Finding, from: Incident | null) => (
    <tr key={f.id} onClick={() => openFinding(f, from)} style={{ cursor: 'pointer' }}>
      ...
      <td style={{ width: 150 }} className="nowrap">
        {fmtTs(f.ts)}
      </td>
      <td style={{ width: 110 }}>
    ...
    <div className="view">
      <div className="view-header">
        <div className="desc">
          <h1>Findings</h1>
          <span className="sub">
            {fmtNum(allIncidents.length)} incident(s) · {fmtNum(active.length)} finding(s) from {enabledCount} enabled rule(s)
            {fpCount ? ` · ${fmtNum(fpCount)} false positive${fpCount === 1 ? '' : 's'}${showFp || status === 'false_positive' ? '' : ' hidden'}` : ''}
            {lastRun ? ` · last run ${fmtTs(lastRun.ts)}` : ' · rules not run yet'}
          </span>
        </div>
        <span className="spacer" />
        <button className="btn ghost sm" onClick={() => setView('rules')}>
          manage rules
        </button>
        <button
          ...
          onClick={() =>
            exportCsv(
              ...
              shownRows.map((f) => ({
                ...
                count: f.count,
                first: f.ts ? new Date(f.ts).toISOString() : '',
                last: f.tsEnd ? new Date(f.tsEnd).toISOString() : '',
                source: f.source,
                status: f.status,
                attack: f.attack.join(' '),
                refs: f.refs.slice(0, 50).join(' '),
                notes: f.notes,
              })),
            )
          }
        ...
        <button className="btn ghost sm" onClick={() => exportJson('findings.json', shownRows)}>
          json
        </button>
      ...
      {running && (
        <div style={{ padding: '6px 16px', background: 'var(--surface)' }} className="col">
          <div className="small dim mono">
            {running.reason === 'ingest' ? 'refreshing findings after ingest · ' : ''}
            {running.done}/{running.total} · {running.rule}
          </div>
          <Progress value={running.done / Math.max(1, running.total)} />
        </div>
      )}
      {!running && staleParts.length > 0 && (
        <div className="bulkbar" style={{ background: 'var(--sev-medium-bg)', borderColor: 'rgba(217,130,43,0.35)', color: 'var(--sev-medium)' }}>
          ...
          <span className="spacer" />
          <button className="btn xs" onClick={run}>
            run rules now
          </button>
        </div>
      )}
      {!running && stale && stale.errors.length > 0 && (
        <div className="bulkbar" style={{ background: 'var(--sev-high-bg)', borderColor: 'rgba(209,64,63,0.35)', color: 'var(--sev-high)' }}>
          <b>
            {stale.errors.length} rule{stale.errors.length === 1 ? '' : 's'} failed in the last run
          </b>
          <span className="ellipsis" title={stale.errors.join('\n')}>
            {stale.errors
              ...
              .map((e) => e.split(':')[0])
              .join(', ')}
            {stale.errors.length > 3 ? ` +${stale.errors.length - 3}` : ''} · their findings are from an earlier run or missing
          </span>
          <span className="spacer" />
          <button className="btn xs" onClick={() => setView('rules')}>
            see diagnostics
          </button>
        </div>
      )}
          ...
          tabs={[
            {
              ...
              label: (
                <span>
                  {group === 'incident' ? 'Incidents' : 'Findings'} <span className="n">{fmtNum(group === 'incident' ? incidents.length : rows.length)}</span>
                </span>
              ),
            },
            {
              ...
              label: (
                <span>
                  ATT&amp;CK <span className="n">{attack.hits.length}</span>
                </span>
              ),
            },
          ]}
        ...
        <>
          <div className="querybar">
            <div className="row wrap">
              ...
              <label className={classNames('pill', sev && 'active')}>
                severity{' '}
              ...
              <label className={classNames('pill', status && 'active')}>
                status{' '}
              ...
              {fpCount > 0 && (
                <button className={classNames('pill', showFp && 'active')} onClick={() => setShowFp(!showFp)} title="false positives are hidden from the queue and the counts by default">
                  {showFp ? 'hiding' : 'show'} {fmtNum(fpCount)} false positive{fpCount === 1 ? '' : 's'}
                </button>
              )}
              <label className={classNames('pill', source && 'active')}>
                source{' '}
                <select value={source} onChange={(e) => setSource(e.target.value)}>
                  <option value="">events + mails</option>
              ...
              <div className="segmented" title="how the queue is grouped">
                {(
                  ...
                  <button
                    key={g}
                    className={classNames(group === g && 'active')}
                    onClick={() => {
                      setGroup(g)
                      setPicked(new Set())
                      setIncident(null)
          ...
          {picked.size > 0 && (
            <div className="bulkbar">
              <b>
                {picked.size} selected{group === 'incident' ? ` · ${pickedFindingIds.length} finding(s)` : ''}
              </b>
              <span className="muted">set status:</span>
              {STATUSES.map((s) => (
                <button key={s} className="btn xs" onClick={() => setStatusFor(pickedFindingIds, s)}>
                  {STATUS_LABEL[s]}
                </button>
              ))}
              <span className="spacer" />
                ...
                {groups.map((g) => (
                  <div key={g.key}>
                    <div className="group-row" onClick={() => toggleGroup(g.key)}>
                      ...
                      <span className="name ellipsis" style={{ maxWidth: 520 }}>
                        {g.label}
                      </span>
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          setPicked(new Set(g.items.map((f) => f.id!)))
                        }}
                      >
                        select all
                      </button>
              ...
              <Flyout
                ...
                meta={
                  <>
                    ...
                    <span>
                      {fmtNum(incident.findings.length)} finding(s) from {incident.rules.length} rule(s)
                    </span>
                    <span>
                      {fmtTs(incident.ts)}
                      {incident.tsEnd && incident.tsEnd !== incident.ts ? ` → ${fmtTs(incident.tsEnd)}` : ''}
                    </span>
                ...
                footer={
                  <>
                    <AddToTimeline
                      ts={incident.ts}
                      text={`${incident.title}: ${incident.lead.title}`}
                      link={{ source: 'findings', id: incident.lead.id!, label: incident.lead.ruleId }}
                    ...
                    <div className="segmented">
                      {STATUSES.map((s) => (
                        <button
                          key={s}
                          className={classNames(incident.status === s && 'active')}
                          onClick={() =>
                            setStatusFor(
                              incident.findings.map((f) => f.id!),
                              s,
                            )
                          }
                        >
                    ...
                    <span className="spacer" />
                    <button className="btn sm" onClick={() => explainIncident(incident)}>
                      ask the analyst
                    </button>
                    {incident.source !== 'mixed' && (
                      <button className="btn sm primary" onClick={() => openRefs(incident.source, incident.refs)}>
                        open {fmtNum(Math.min(incident.refs.length, REFS_OPEN))} row(s)
                      </button>
                    )}
                  </>
                }
              >
                ...
                <div className="section">
                  ...
                  <div className="small muted">
                    {incident.subtitle}.{' '}
                    {incident.kind === 'chain'
                      ? 'These findings have their rows among the steps of the attack chain: the verdict on the Review page decides them together, and a finding can be unlinked there to be decided on its own.'
                ...
                <div className="section">
                  ...
                  <div className="highlight">
                    {Object.entries(incident.entities)
                      .slice(0, 12)
                      .map(([k, v]) => (
                        <div className="f" key={k}>
                          ...
                          <span
                            className="v"
                            onClick={() => openEntity(k, String(v), incident.source === 'mails' ? 'mails' : 'events')}
                            title={entityKind(k) ? `open the ${entityKind(k)} page` : `pivot on ${k}`}
                          >
                            {String(v)}
                          </span>
                        </div>
                    ...
                    {incident.kind === 'mail' && (
                      <div className="f">
                        ...
                        <span
                          className="v"
                          onClick={() => {
                            setFocus({ source: 'mails', id: incident.refs[0] })
                            setView('mails')
                          }}
                        >
                          open #{incident.refs[0]}
                        </span>
                      </div>
                    )}
                  </div>
                  {incident.attack.length > 0 && (
                    <div className="row wrap" style={{ gap: 6 }}>
                      {incident.attack.map((t) =>
                        attackHref(t) ? (
                          <a key={t} className="badge outline" href={attackHref(t)} target="_blank" rel="noreferrer noopener">
                            {t}
                          </a>
                        ) : (
                          <Badge key={t} sev="outline">
              ...
              <Flyout
                title={
                  <span className="row" style={{ gap: 8 }}>
                    ...
                    <FindingSeverity finding={selected} />
                ...
                meta={
                  <>
                    ...
                    <span>
                      {fmtTs(selected.ts)}
                      {selected.tsEnd && selected.tsEnd !== selected.ts ? ` → ${fmtTs(selected.tsEnd)}` : ''}
                    </span>
                    ...
                    {inheritedReview(selected) && (
                      <span title="the status was set on an earlier evaluation of this finding key and carried over by the last run">status carried over from {fmtTs(selected.createdAt)}</span>
                    )}
                  </>
                }
                tabs={
                ...
                footer={
                  <>
                    ...
                    <div className="segmented">
                      {STATUSES.map((s) => (
                        <button key={s} className={classNames(selected.status === s && 'active')} onClick={() => setStatusFor([selected.id!], s)}>
                          {STATUS_LABEL[s]}
                        </button>
                      ))}
                    </div>
                    <span className="spacer" />
                    <button className="btn sm" onClick={() => explain(selected)}>
                      ask the analyst
                    </button>
                    <button className="btn sm primary" onClick={() => openRefs(selected.source, selected.refs)}>
                      open {fmtNum(Math.min(selected.refs.length, REFS_OPEN))} row(s)
                    </button>
                  </>
                }
              >
                ...
                {flyTab === 'overview' && (
                  <>
                    <div className="section">
                      ...
                      {selected.escalation && (
                        <div className="small" style={{ color: 'var(--danger)' }}>
                          escalated by: {selected.escalation}
                        </div>
                      )}
                      {lastRun?.errors?.some((e) => e.startsWith(selected.ruleId)) && (
                    ...
                    <div className="section">
                      ...
                      <div className="highlight">
                        {Object.entries(selected.entities).map(([k, v]) => (
                          <div className="f" key={k}>
                            <span className="k">{k}</span>
                            <span className="v" onClick={() => openEntity(k, String(v), selected.source)} title={entityKind(k) ? `open the ${entityKind(k)} page` : `pivot on ${k}`}>
                              {String(v)}
                            </span>
                          </div>
                        ))}
                        <div className="f">
                          <span className="k">referenced rows</span>
                          <span className="v" onClick={() => openRefs(selected.source, selected.refs)}>
                            {fmtNum(selected.refs.length)}
                            {selected.refs.length > REFS_OPEN ? ` (first ${REFS_OPEN} open)` : ''}
                          </span>
                        </div>
                        {selected.refs.slice(0, 1).map((id) => (
                          <div className="f" key={id}>
                            <span className="k">first row</span>
                            <span
                              className="v"
                              onClick={() => {
                                setFocus({ source: selected.source, id })
                                setView(selected.source)
                              }}
                            >
                              #{id}
                            </span>
                    ...
                    <div className="section">
                      <h3>Insights</h3>
                      <div className="kv">
                        {Object.entries(selected.entities)
                          .slice(0, 4)
                          .map(([k, v]) => (
                            <div key={k} style={{ display: 'contents' }}>
                              <div className="k">{k} in this case</div>
                              <div className="v">
                                {prevalence[k] == null ? <span className="muted">…</span> : `${fmtNum(prevalence[k]!)} ${selected.source === 'events' ? 'event(s)' : 'mail(s)'} with ${String(v)}`}
                              </div>
                            </div>
                          ))}
                        {(() => {
                          const rc = relatedChains(selected, all)
                          return rc.length ? (
                            <>
                        ...
                        {(() => {
                          const inc = buildIncidents(all).find((i) => i.findings.some((f) => f.id === selected.id))
                          return inc && inc.findings.length > 1 ? (
                            <>
                              ...
                              >
                                {inc.title} · {inc.findings.length} finding(s)
                              </div>
                            </>
                          ) : null
                        })()}
                        ...
                        <div className="v">
                          {fmtNum(all.filter((f) => f.ruleId === selected.ruleId).length)} finding(s) ·{' '}
                          {fmtNum(all.filter((f) => f.ruleId === selected.ruleId && f.status === 'false_positive').length)} marked false positive
                        </div>
                      </div>
                    </div>
                    <div className="section">
                      ...
                      <textarea
                        ...
                        onBlur={(e) => {
                          getDb().findings.update(selected.id!, { notes: e.target.value })
                          reload()
                        }}
                      />
                    </div>
                ...
                {flyTab === 'table' && (
                  <div className="kv">
                    {Object.entries(selected)
                      .filter(([k, v]) => !['id', 'caseId', 'entities', 'refs'].includes(k) && v != null && v !== '' && !(Array.isArray(v) && !v.length))
                      .map(([k, v]) => (
                        <div key={k} style={{ display: 'contents' }}>
                    ...
                    {Object.entries(selected.entities).map(([k, v]) => (
                      <div key={'e' + k} style={{ display: 'contents' }}>
                        <div className="k">entity.{k}</div>
                        <div className="v click" onClick={() => pivot(String(v), k, selected.source)}>
                          {String(v)}
                        </div>
                      </div>
                    ))}
          ...
          <div className="panel">
            <div className="panel-h">
              ...
              <span className="muted">
                · {attack.hits.length} technique(s) across {fmtNum(all.length)} finding(s)
              </span>
            </div>
              ...
              <tbody>
                {attack.hits.map((h) => (
                  <tr key={h.id}>
                    ...
                    <td className="sans">
                      {attackHref(h.id) ? (
                        <a href={attackHref(h.id)} target="_blank" rel="noreferrer noopener">
                          attack.mitre.org
                        </a>
                      ) : (
                        <span className="muted">not a technique id</span>
                      )}{' '}
                      ·{' '}
                      <button
                        className="btn link"
                ...
                {!attack.hits.length && (
                  <tr>
                    <td colSpan={6} className="muted sans">
                      no technique yet - run the rules
                    </td>
                  </tr>
                )}
              </tbody>
