  ...
  return (
    <div className="view">
      <div className="view-header">
        <div className="desc">
          <h1>Attack chains</h1>
          <span className="sub">{res ? `${res.chains.length} chain(s) · ${Object.entries(sevCounts).map(([k, v]) => `${v} ${k}`).join(', ') || 'none'} · built ${res.builtAt ? fmtTs(res.builtAt) : ''}` : 'not built yet - run the rules first so their findings attach to the steps'}</span>
        </div>
        <span className="spacer" />
        <label className="row" style={{ gap: 6 }}><span className="muted small">seed risk ≥</span><input className="input" type="number" style={{ width: 60 }} value={minRisk} min={0} max={100} onChange={(e) => setMinRisk(Number(e.target.value))} /></label>
        <label className="row" style={{ gap: 6 }}><span className="muted small">window</span><input className="input" type="number" style={{ width: 60 }} value={hours} min={1} max={720} onChange={(e) => setHours(Number(e.target.value))} /><span className="muted small">h</span></label>
        ...
        <div className="left">
          {!res && (
            <div className="muted small" style={{ padding: 14, lineHeight: 1.5 }}>
              A chain starts from a suspicious mail (risk above the threshold, or carrying a medium+ finding) and follows what its recipient did next across the mailbox, Microsoft 365 / Entra audit rows and Windows events: replies to the sender, sign-ins, mailbox rules, consent grants, role changes, processes spawned by Outlook or a browser, DNS queries and files that name the mail's links or attachments.
            </div>
          )}
          ...
          {res?.chains.map((c) => (
            <div key={c.id} className={'group-row' + (open === c.id ? ' active' : '')} style={{ display: 'grid', gridTemplateColumns: '14px 1fr auto', gap: '2px 10px', padding: '10px 14px', cursor: 'pointer', alignItems: 'start' }} onClick={() => { setOpen(c.id); setStepIdx(null); setTab('story') }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div className="ellipsis small" style={{ color: 'var(--fg-2)' }}>{c.seed.subject || '(no subject)'}</div>
                <div className="small mono" style={{ color: 'var(--fg-3)' }}>{fmtTs(c.start)} · {span(c.end - c.start)}</div>
              </div>
              <div className="col" style={{ alignItems: 'flex-end', gap: 2 }}>
                <span className="mono" style={{ fontWeight: 600, color: 'var(--fg-1)' }}>{c.score}</span>
                <span className="small mono" style={{ color: 'var(--fg-3)' }}>{c.steps.length} steps</span>
                <span className="small mono" style={{ color: c.artifactLinks ? 'var(--accent)' : 'var(--fg-3)' }}>{c.artifactLinks} links</span>
              </div>
            </div>
          ))}
        </div>
        <div className="right">
          {!chain && <div className="muted" style={{ padding: 24 }}>{res?.chains.length ? 'select a chain' : ''}</div>}
          {chain && (
            <>
              <div style={{ padding: '12px 16px 8px', borderBottom: '1px solid var(--line)', background: 'var(--surface)' }}>
                ...
                <div className="row wrap small" style={{ gap: 6, marginTop: 8 }}>
                  ...
                  {chain.entities.ips.map((ip) => <span key={ip} className="pill" onClick={() => setEntity({ kind: 'ip', value: ip })} title="IP seen in the chain">{ip}</span>)}
                  {chain.entities.hosts.map((h) => <span key={h} className="pill" onClick={() => setEntity({ kind: 'host', value: h })} title="host seen in the chain"><IconHost /> {h}</span>)}
                  {chain.entities.domains.slice(0, 6).map((d) => <span key={d} className="pill" onClick={() => setEntity({ kind: 'domain', value: d })} title="domain from the seed mail">{d}</span>)}
              ...
              {tab === 'story' && (
                <div className="pane" style={{ flex: 1, height: 'auto', borderTop: 0, gridTemplateColumns: step ? '1fr 340px' : '1fr' }}>
                  <div className="pane-main" style={{ overflow: 'auto' }}>
                    <div className="story">
                      <div className="step" onClick={() => { setFocus({ source: 'mails', id: chain.seed.id }); setView('mails') }} title="open the seed mail">
                        <span className="t">{fmtTs(chain.seed.ts)}<br /><span style={{ color: 'var(--accent)' }}>seed</span></span>
                        <span className="n mail" title="mailbox"><IconMail /></span>
                        <span>
                          <div className="title">{chain.seed.subject || '(no subject)'} <Badge sev={chain.seed.risk >= 80 ? 'critical' : chain.seed.risk >= 60 ? 'high' : 'medium'}>risk {chain.seed.risk}</Badge></div>
                          <div className="sub">from {chain.seed.fromAddr}{chain.seed.urlDomains.length ? ` · links ${chain.seed.urlDomains.join(', ')}` : ''}{chain.seed.attachments.length ? ` · attachments ${chain.seed.attachments.join(', ')}` : ''}</div>
                          <div className="row wrap" style={{ gap: 4, marginTop: 3 }}>{chain.seed.findings.map((f) => <Badge key={f.ruleId} sev={f.severity} title={f.title}>{f.ruleId}</Badge>)}</div>
                        </span>
                      </div>
                      {chain.steps.map((s, i) => {
                        const k = stepKind(s)
                        ...
                        return (
                          <div key={i} className={'step' + (stepIdx === i ? ' active' : '')} onClick={() => setStepIdx(i)}>
                            <span className="t">{fmtTs(s.ts)}<br /><span>{offset(s.offsetMin)}</span></span>
                            <span className={'n ' + k} title={KIND_LABEL[k]}><Icon /></span>
                            <span>
                              <div className="title">{s.title}{s.count > 1 && <span className="muted" style={{ fontWeight: 400 }}> ×{fmtNum(s.count)}</span>}</div>
                              <div className="sub">{[s.computer, s.ipAddress].filter(Boolean).join(' · ')}{s.tsEnd && s.tsEnd > s.ts ? ` · until ${fmtTs(s.tsEnd)}` : ''}</div>
                              {(s.artifacts.length > 0 || s.findings.length > 0) && (
                                <div className="row wrap" style={{ gap: 4, marginTop: 3 }}>
                                  {s.artifacts.map((a, j) => <Badge key={j} sev={strongArtifact(a) ? 'critical' : 'high'} title="what ties this step to the seed mail">{a}</Badge>)}
                                  {s.findings.map((f) => <Badge key={f.ruleId} sev={f.severity} title={f.title}>{f.ruleId}</Badge>)}
                                </div>
                              )}
                            </span>
                          </div>
              ...
              {tab === 'entities' && (
                <div className="view-body">
                  <div className="section">
                    <h3>Seed mail</h3>
                    <div className="kv">
                      <div className="k">subject</div><div className="v click" onClick={() => { setFocus({ source: 'mails', id: chain.seed.id }); setView('mails') }}>{chain.seed.subject || '(no subject)'}</div>
                      <div className="k">from</div><div className="v click" onClick={() => chain.seed.fromAddr && setEntity({ kind: 'address', value: chain.seed.fromAddr })}>{chain.seed.fromAddr}</div>
                      <div className="k">date</div><div className="v">{fmtTs(chain.seed.ts)}</div>
                      <div className="k">risk</div><div className="v">{chain.seed.risk}</div>
                      <div className="k">flags</div><div className="v">{chain.seed.flags.join(', ') || '—'}</div>
                      <div className="k">links</div><div className="v">{chain.seed.urlDomains.join(', ') || '—'}</div>
                      <div className="k">attachments</div><div className="v">{chain.seed.attachments.join(', ') || '—'}</div>
                      <div className="k">findings</div><div className="v">{chain.seed.findings.map((f) => `${f.ruleId} (${f.severity})`).join(', ') || '—'}</div>
                    </div>
                  </div>
                  {(chain.relatedSeeds?.length ?? 0) > 0 && (
                    <div className="section" style={{ marginTop: 16 }}>
                      ...
                      <table className="table compact">
                        <thead><tr><th>date (UTC)</th><th>risk</th><th>from</th><th>subject</th></tr></thead>
                        <tbody>
                          {chain.relatedSeeds!.map((s) => (
                            <tr key={s.id} style={{ cursor: 'pointer' }} onClick={() => { setFocus({ source: 'mails', id: s.id }); setView('mails') }}>
                              <td className="nowrap">{fmtTs(s.ts)}</td><td>{s.risk}</td><td>{s.fromAddr}</td><td className="sans">{s.subject}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              {tab === 'json' && <div className="view-body"><pre className="codeblock">{JSON.stringify(chain, null, 2)}</pre></div>}
            </>
          )}
        </div>
      </div>
