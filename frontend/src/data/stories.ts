...
export async function selectRows(
      ...
      .each((e) => {
        ...
        if (!named && !fromIp && !onHost) {
          if (flaggedHost && dnsAnswer(row)) {
            if (++dnsPicked > DNS_CAP) count('dns')
        ...
        if (++picked > eventCap) {
          count('context')
          count(named ? 'context-identities' : fromIp ? 'context-addresses' : 'context-hosts')
          return
        }
  ...
  const kept = new Set(mails.map((m) => m.id))
  const replies = allMails.filter((m) => !kept.has(m.id) && inWindow(m.date) && names.has(accountName(m.fromAddr) ?? ''))
  if (replies.length > MAIL_CAP) count('replies', replies.length - MAIL_CAP)
...
export function storyCoverageWarnings(stats: StoryResult['stats'] | undefined): string[] {
  ...
  const labels: Record<string, string> = {
    ...
    identities: `The records naming the flagged people passed ${num(EVENT_CAP)}: the stories read the first ones in time.`,
    addresses: `The records from the flagged addresses passed ${num(EVENT_CAP)}: the stories read the first ones in time.`,
    hosts: `The logons, processes and services on the flagged hosts passed ${num(EVENT_CAP)}: the stories read the first ones in time.`,
    context: `The records around the flags passed ${num(EVENT_CAP)}, which a browser case reads under one cap for the flagged people, addresses and hosts together: the stories read the first ones in time${context.length ? ` and left out ${context.join(', ')}` : ''}.`,
    ...
    replies: `Only the first ${num(MAIL_CAP)} mails the flagged people sent were read.`,
    dns: `The DNS answers on the flagged hosts passed ${num(DNS_CAP)}: the addresses they give to hosts come from the first ones in time.`,
