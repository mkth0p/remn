...
describe('queries over IndexedDB', () => {
  beforeAll(async () => {
    ...
    for (let i = 0; i < 300; i++) {
      events.push({ caseId: 1, evidenceId: 1, ts: T0 + i * 60_000, eventId: i % 3 === 0 ? 4625 : 4624, provider: 'Security', channel: 'Security', computer: i % 2 ? 'WS01' : 'WS02', targetUser: i % 5 === 0 ? 'admin' : 'bob', ipAddress: `10.0.0.${i % 7}`, summary: `event ${i}`, data: { X: i } })
    }
    await db.events.bulkAdd(events)
    const mails: MailRow[] = [
      { caseId: 1, evidenceId: 2, folder: 'Inbox', subject: 'URGENT invoice', date: T0, fromName: 'Marie', fromNameNorm: 'marie', fromAddr: 'm@interne-fr.co', fromDomain: 'interne-fr.co', fromRegistrable: 'interne-fr.co', replyTo: [], returnPath: null, to: [], cc: [], bcc: [], recipientCount: 0, messageId: null, originIp: '185.220.101.4', hopCount: 2, hops: [], auth: {}, keywordHits: {}, urls: [], urlCount: 0, attachments: [{ name: 'a.docm', ext: 'docm', realExt: 'docx', size: 10, sha256: 'x', md5: 'y', risk: 90, flags: ['office_macro'] }], attachmentCount: 1, maxAttachmentRisk: 90, lookalike: {}, flags: ['spf_fail', 'att_office_macro'], risk: 95 },
      { caseId: 1, evidenceId: 2, folder: 'Inbox', subject: 'hello', date: null, fromName: 'Bob', fromNameNorm: 'bob', fromAddr: 'b@x.com', fromDomain: 'x.com', fromRegistrable: 'x.com', replyTo: [], returnPath: null, to: [], cc: [], bcc: [], recipientCount: 0, messageId: null, originIp: null, hopCount: 0, hops: [], auth: {}, keywordHits: {}, urls: [], urlCount: 0, attachments: [], attachmentCount: 0, maxAttachmentRisk: 0, lookalike: {}, flags: [], risk: 0 },
    ]
