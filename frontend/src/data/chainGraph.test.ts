...
const M = 60_000
const step = (p: Partial<ChainStep> & { title: string; ts: number }): ChainStep => ({ kind: 'event', source: 'events', id: null, count: 1, weight: 1, artifacts: [], findings: [], offsetMin: (p.ts - 1000) / M, tsEnd: p.ts, origin: 'm365', ...p })
const chain = (p: Partial<Chain> = {}): Chain => ({
  id: 'c1', identity: 'alice', identityLabel: 'alice@corp.test', start: 1000, end: 1000 + 60 * M, score: 90, severity: 'critical', artifactLinks: 2, summary: 's',
  seed: { id: 7, ts: 1000, subject: 'Urgent invoice', fromAddr: 'ceo@evil.test', risk: 90, flags: [], findings: [{ ruleId: 'mail-credential-phishing', title: 'Credential phishing', severity: 'critical' }], urlDomains: ['evil-login.net'], attachments: ['invoice.html'] },
  ...
  steps: [
    ...
    step({ title: 'reply to the sender: RE: Urgent invoice', ts: 1000 + 5 * M, kind: 'mail', source: 'mails', id: 8, weight: 4, artifacts: ['victim engaged with the sender', 'same thread'], origin: undefined }),
    ...
    step({ title: 'inbox rule created: forward to drop@evil.test', ts: 1000 + 20 * M, ipAddress: '203.0.113.9', weight: 5, artifacts: ['forwarding rule'], findings: [{ ruleId: 'm365-inbox-rule-forwarding', title: 'Inbox rule forwards mail', severity: 'high' }] }),
  ],
  ...p,
})
...
describe('buildCampaignGraph', () => {
  it('marks infrastructure shared by several chains and lists it as an insight', () => {
    const a = chain({ id: 'a' })
    const b = chain({ id: 'b', identityLabel: 'bob@corp.test', entities: { user: 'bob@corp.test', ips: ['203.0.113.9', '198.51.100.7'], hosts: [], attackerAddresses: ['ceo@evil.test'], domains: ['other.test'] } })
...
describe('folding', () => {
  it('folds consecutive repeats of the same action even when they carry findings, and never folds a step tied to the mail', () => {
    ...
    for (let i = 0; i < 30; i++) steps.push(step({ title: `sign-in from NL via Other clients to Office 365 (${i})`, ts: 1000 + i * M, ipAddress: '203.0.113.9', weight: 2, findings: [{ ruleId: 'm365-signin-risky', title: 'Risky sign-in', severity: 'high' }] }))
    ...
    for (let i = 0; i < 30; i++) steps.push(step({ title: 'mailbox items accessed (Sync)', ts: 1000 + (50 + i) * M, ipAddress: '203.0.113.9', weight: 2, findings: [{ ruleId: 'm365-mailitemsaccessed-burst', title: 'Burst', severity: 'medium' }] }))
