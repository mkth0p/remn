...
def test_white_background_is_not_hidden_text():
    ...
    html = ('<html style="background-color: #fff; color: #262626"><body style="background-color:#ffffff">'
...
def test_data_scripts_are_not_html_script():
    card = ('<html><body><script type="application/adaptivecard+json">{"type":"AdaptiveCard"}</script>'
            "<p>Hi</p></body></html>")
...
def test_exchange_internal_mail_is_trusted():
    raw = _raw([
        ("Received", "from PAVPR05MB10277.eurprd05.prod.outlook.com (2603:10a6:102:2f8::21) by "
        ...
        ("From", "Jean Dupont <jean.dupont@interne.fr>"), ("To", "Marie Lefevre <marie.lefevre@interne.fr>"),
        ("Subject", "Point budget"), ("Date", "Tue, 1 Sep 2026 11:00:00 +0200"),
...
def test_gateway_verdicts_and_helo_gating():
    base = [("From", "News <news@vendor-news.com>"), ("To", "<user@interne.fr>"), ("Subject", "Offre"),
            ("Date", "Tue, 1 Sep 2026 11:00:00 +0200"), ("Message-ID", "<1@vendor-news.com>")]
    spam = _raw(base + [
        ("Received", "from mail.vendor-news.com ([185.220.101.4]) by DB1PEPF0003922E.mail.protection.outlook.com "
                     "with SMTP; Tue, 1 Sep 2026 09:00:00 +0000"),
        ("Authentication-Results", "spf=pass (sender IP is 185.220.101.4) smtp.mailfrom=vendor-news.com; "
    ...
    exo = _raw(base + [("Received", "from VI1PR05MB5821.eurprd05.prod.outlook.com (2603:10a6:803:1::20) by "
                                    "X.eurprd05.prod.outlook.com with HTTPS; Tue, 1 Sep 2026 09:00:00 +0000")], "<p>x</p>")
...
def test_brand_owned_domains_are_not_lookalikes():
    for d in ("teams.mail.microsoft", "engage.mail.microsoft", "contoso.onmicrosoft.com", "service-now.com",
              "credit-agricole.fr", "docs.google.com"):
...
def test_calendar_items_have_no_undisclosed_recipients():
    headers = [("From", "Jean Dupont <jean.dupont@interne.fr>"), ("Subject", "Point hebdo"),
               ("Date", "Tue, 1 Sep 2026 11:00:00 +0200")]
    row = build_row(headers, "Salle 3", None, [], CTX, folder="Calendrier", size=None,
...
def test_authenticated_vendor_mail_keeps_its_own_links():
    body = ('<html><body><div style="display:none">' + ("Security alert " * 30) + "</div>"
    ...
    raw = _raw([
        ...
        ("Authentication-Results", "spf=pass smtp.mailfrom=microsoft.com; dkim=pass header.d=microsoft.com; dmarc=pass action=none header.from=microsoft.com"),
        ("From", "Microsoft account team <account-security-noreply@accountprotection.microsoft.com>"), ("To", "<user@interne.fr>"),
        ("Subject", "Security info was added"), ("Date", "Tue, 1 Sep 2026 11:00:00 +0200"), ("Message-ID", "<1@accountprotection.microsoft.com>"),
    ], body)
    row = parse_message_bytes(raw, CTX)
    f = set(row["flags"])
    ...
    spoof = _raw([
        ...
        ("From", "Support <support@evil-login.net>"), ("To", "<user@interne.fr>"), ("Subject", "Verify"),
        ("Date", "Tue, 1 Sep 2026 11:00:00 +0200"), ("Message-ID", "<2@evil-login.net>"),
...
def test_authenticated_phpmailer_newsletter_is_not_forged():
    raw = _raw([
        ...
        ("From", "Vendor <news@vendor-news.com>"), ("To", "<user@interne.fr>"), ("Subject", "Newsletter"),
        ("Date", "Tue, 1 Sep 2026 11:00:00 +0200"), ("Message-ID", "<2@vendor-news.com>"),
    ...
    raw2 = _raw([
        ...
        ("X-Mailer", "PHPMailer 6.8.0"), ("From", "Vendor <news@vendor-news.com>"), ("To", "<user@interne.fr>"),
        ("Subject", "Newsletter"), ("Date", "Tue, 1 Sep 2026 11:00:00 +0200"), ("Message-ID", "<3@vendor-news.com>"),
    ], "<p>x</p>")
