...
from mail_calibration import SETTINGS, examples, message
...
from services.store.casestore import MAIL_COLUMNS, StoreRegistry, _arrow_table
...
def test_benign_capabilities_and_malicious_controls_through_rules(store):
    ...
    rules = [
        r
        for p in (Path(__file__).resolve().parents[2] / "rules/mail").glob("*.yaml")
        for r in yaml.safe_load_all(p.read_text(encoding="utf-8"))
...
def test_old_html_facts_can_be_rescored_and_missing_facts_are_explicit():
    old = {
        "name": "report.html",
        "risk": 100,
        "flags": ["html_smuggling", "html_script", "html_attachment"],
...
def test_unaligned_spf_never_grants_internal_sender_trust():
    r = message("Invoice")
    r.update(
        fromAddr="finance@interne.fr",
        fromDomain="interne.fr",
        fromRegistrable="interne.fr",
        auth={"spf": "pass", "spfDomain": "unrelated.example"},
        flags=["spf_domain_unaligned", "returnpath_mismatch"],
    )
...
def test_grouped_escalation_keeps_critical_evidence_beyond_ref_cap(store, window):
    ...
    for i in range(MAX_REFS + 2):
        w.add(
            {"date": i, "fromAddr": "sender@example.org", "subject": "file", "flags": ["att_macro_vba_stomping"] if i == MAX_REFS + 1 else ["att_office_macro"]}
        )
    w.flush()
    rule = {
        "id": "macro-review",
