...
def _ual(op: str, workload: str, user: str, ip: str, minutes: float, day: int = 0, **extra: Any) -> dict[str, Any]:
    rec = {
        ...
        "ResultStatus": extra.pop("ResultStatus", "Succeeded"),
        "UserKey": user,
        "UserType": 0,
        "Version": 1,
        "Workload": workload,
...
def ual_records() -> list[dict[str, Any]]:
    ...
    for i in range(6):
        recs.append(
            _ual(
                ...
                RecordType=15,
                ExtendedProperties=[
                    {"Name": "UserAgent", "Value": "Mozilla/5.0 (Windows NT 10.0) Edge/128"},
                    {"Name": "RequestType", "Value": "OAuth2:Authorize"},
                    {"Name": "ResultStatusDetail", "Value": "Success"},
                ],
                Actor=[{"ID": VICTIM, "Type": 5}],
                Target=[{"ID": "Office 365 Exchange Online", "Type": 0}],
                ObjectId="Office 365 Exchange Online",
            )
        )
        recs.append(
            _ual(
                ...
                -110 + i * 15,
                RecordType=6,
                ObjectId=f"https://contoso.sharepoint.com/sites/finance/Shared Documents/report-{i}.xlsx",
                SiteUrl="https://contoso.sharepoint.com/sites/finance/",
                SourceFileName=f"report-{i}.xlsx",
    ...
    recs.append(
        _ual(
            ...
            31,
            RecordType=1,
            ObjectId="EURPR05A001.prod.outlook.com/Microsoft Exchange Hosted Organizations/contoso.onmicrosoft.com/alice\\.",
            ExternalAccess=False,
            OriginatingServer="PAVPR05MB10277 (15.20.4200.000)",
            Parameters=[
                {"Name": "Name", "Value": "."},
                {"Name": "ForwardTo", "Value": "attacker@proton-mail.example"},
    ...
    recs.append(
        _ual(
            ...
            40,
            RecordType=1,
            ObjectId="contoso.onmicrosoft.com/alice",
            Parameters=[
                {"Name": "Identity", "Value": VICTIM},
                {"Name": "ForwardingSmtpAddress", "Value": "smtp:attacker@proton-mail.example"},
    ...
    recs.append(
        _ual(
            ...
            62,
            RecordType=8,
            ObjectId="Mail Reader Pro",
            Actor=[{"ID": VICTIM, "Type": 5}],
            Target=[{"ID": "Mail Reader Pro", "Type": 1}, {"ID": "ServicePrincipal_deadbeef", "Type": 2}],
            ModifiedProperties=[
                {"Name": "ConsentContext.IsAdminConsent", "NewValue": "False", "OldValue": ""},
                {
                    "Name": "ConsentAction.Permissions",
                    "NewValue": "[] => [[Id: aaaa, ClientId: bbbb, PrincipalId: cccc, ResourceId: dddd, ConsentType: Principal, Scope: Mail.Read offline_access User.Read]]",
                    "OldValue": "",
                },
            ],
        )
    ...
    recs.append(
        _ual(
            ...
            ObjectId=ADMIN,
            Actor=[{"ID": VICTIM, "Type": 5}],
            Target=[{"ID": ADMIN, "Type": 5}, {"ID": "User_" + ADMIN, "Type": 2}],
            ModifiedProperties=[
...
def entra_signins() -> list[dict[str, Any]]:
    def si(
        ...
        return {
            ...
            "userAgent": "Mozilla/5.0" if client == "Browser" else "BAV2ROPC",
            "status": {
                "errorCode": err,
                "failureReason": reason or ("Other." if err == 0 else "Error validating credentials due to invalid username or password."),
            ...
            "deviceDetail": {
                ...
                "browser": "Edge 128.0.0" if client == "Browser" else None,
                "isCompliant": False,
                "isManaged": False,
            },
...
def write_all(out_dir: str) -> dict[str, str]:
    ...
    with open(p, "w", newline="", encoding="utf-8") as fh:
        ...
        for s in signins:
            ...
            w.writerow(
                [
                    s["createdDateTime"].replace("T", " ").rstrip("Z"),
                    s["id"],
                    s["userAgent"],
                    "c-1",
                    s["conditionalAccessStatus"],
                    "Yes",
                    s["userPrincipalName"],
                    s["userId"],
                    s["userPrincipalName"],
                    "Success" if s["status"]["errorCode"] == 0 else "Failure",
                    s["status"]["failureReason"] if s["status"]["errorCode"] else "",
                    s["ipAddress"],
                    f"{loc['city']}, , {loc['countryOrRegion']}",
                    s["appDisplayName"],
                    s["appId"],
                    s["resourceDisplayName"],
                    s["clientAppUsed"],
                    s["deviceDetail"]["browser"] or "",
