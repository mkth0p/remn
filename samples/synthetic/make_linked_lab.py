...
WORKDAYS = [datetime(2026, 8, 17, 8, tzinfo=timezone.utc) + timedelta(days=i) for i in range(19) if (datetime(2026, 8, 17) + timedelta(days=i)).weekday() < 5]
PROVIDERS = {
...
SETTINGS = {"internalDomains": [ORG], "expectedCountries": ["FR"], "vipNames": ["marie lefevre"],
            "adminAccounts": ["lab.admin"], "serviceAccounts": ["svc.backup", "svc.monitor"],
            "internalIps": ["10.0.0.0/8", "127.0.0.0/8", "::1"], "trustedSenders": [], "brands": [],
            "businessHours": {"start": 8, "end": 19, "tz": "Europe/Paris"}, "weekendDays": [0, 6],
            "deepAttachments": True, "keepBodies": True, "includeRaw": True, "networkAllowed": False}
...
def pdf_report(js=False):
    content = b"BT /F1 12 Tf 50 780 Td (Northstar synthetic supplier report) Tj 0 -24 Td (Total approved: 42 units. Reference REMN-LAB. No action required.) Tj ET"
    objs = [b"<< /Type /Catalog /Pages 2 0 R" + (b" /OpenAction 6 0 R" if js else b"") + b" >>",
...
def message_bytes(m):
    ...
    dom = frm.split("@")[-1]
    date = format_datetime(datetime.fromtimestamp(t, timezone.utc))
    auth = "fail" if m.get("auth_fail") else "pass"
    headers = [f'From: "{m.get("display", frm.split("@")[0].replace(".", " ").title())}" <{frm}>', f"To: {to}",
               f'Subject: {m["subject"]}', f"Date: {date}", f"Message-ID: <{key}@remn-lab.example>",
               f"Return-Path: <{frm}>", f'Received: from smtp.{dom} ([{m.get("ip", "198.51.100.10")}]) by mx.{ORG} with ESMTPS id {key}; {date}',
               f"Authentication-Results: mx.{ORG}; spf={auth} smtp.mailfrom={dom}; dkim={auth} header.d={dom}; dmarc={auth} header.from={dom}",
               "X-REMN-Synthetic: linked-investigation-v1", "MIME-Version: 1.0"]
    ...
    boundary = "REMN_" + key
    parts = [f'Content-Type: multipart/mixed; boundary="{boundary}"', "", f"--{boundary}",
             "Content-Type: text/plain; charset=utf-8", "Content-Transfer-Encoding: 8bit", "", m["body"]]
    if m.get("html"):
    ...
    for name, data, mime in m.get("attachments", []):
        ...
        parts += [f"--{boundary}", f"Content-Type: {mime}", f'Content-Disposition: attachment; filename="{name}"',
...
def normal_mails(n):
    subjects = ["Project status", "Meeting notes", "Supplier invoice", "Weekly report", "Delivery schedule", "Approved purchase order", "IT service update", "Training invitation"]
    for i in range(n):
        ...
        body += "\n".join(f"Workstream {j}: task {(i * 17 + j) % 997} is complete; owner team-{i % 12}; accepted quantity {j * 3}; next review follows the usual approval process. Document /projects/{i % 80}/week-{i % 4}." for j in range(1, 18))
...
def normal_windows(channel, n):
    for i in range(n):
        ...
        if channel == "Security":
            ...
            data = {"TargetUserName": user, "TargetDomainName": "NORTHSTAR", "LogonType": 3, "IpAddress": ip,
                    "AuthenticationPackageName": "Kerberos", "WorkstationName": host, "TargetLogonId": hex(i + 4096)}
            if eid == 4625:
                data.update(Status="0xc000006d", SubStatus="0xc000006a")
        elif channel == "Sysmon":
            eid = [1, 3, 22, 11][i % 4]
            data = {"User": "NORTHSTAR\\" + user, "ProcessGuid": "{" + uid(f"BG-P-{i}") + "}", "Image": r"C:\Program Files\Microsoft\Edge\Application\msedge.exe"}
            if eid == 1:
                data.update(CommandLine='msedge.exe https://portal.northstar.example/dashboard', ParentImage=r"C:\Windows\explorer.exe", ProcessId=3000+i%5000)
            elif eid == 3:
            ...
            eid = 4104
            data = {"User": "NORTHSTAR\\svc.monitor", "ScriptBlockText": f"Write-Output 'Routine inventory run {i}: WS-{i%180+1:03d}'", "ScriptBlockId": uid(f"BG-PS-{i}"), "MessageNumber": 1, "MessageTotal": 1, "Path": r"C:\Program Files\Northstar\Inventory.ps1"}
        elif channel == "System":
            eid = 7036
...
def ual(key, t, user, op, workload="Exchange", ip="198.51.100.10", **extra):
    raw = {"CreationTime": iso(t), "Id": uid(key), "Operation": op, "Workload": workload, "UserId": user,
           "OrganizationId": TENANT, "RecordType": 1, "ResultStatus": "Succeeded", "UserType": 0, "UserKey": user,
           "ClientIP": ip, "Version": 1, "SyntheticCase": "REMN-LAB", "SyntheticRecord": key}
    raw.update(extra)
    return {"t": t, "key": key, "raw": raw}


def signin(key, t, user, ip="198.51.100.10", country="FR", err=0, risk="none", client="Browser", **extra):
    raw = {"id": uid(key), "createdDateTime": iso(t), "userPrincipalName": user, "userDisplayName": user.split("@")[0],
           "appDisplayName": "Office 365 Exchange Online", "appId": "00000002-0000-0ff1-ce00-000000000000", "userId": uid(user),
           "ipAddress": ip, "clientAppUsed": client, "conditionalAccessStatus": "success" if not err else "failure", "isInteractive": client == "Browser",
           "riskLevelDuringSignIn": risk, "riskLevelAggregated": risk, "riskState": "atRisk" if risk != "none" else "none",
           ...
           "deviceDetail": {"deviceId": uid(user+"device"), "displayName": "WS-"+user.split("@")[0], "operatingSystem": "Windows 11", "browser": "Edge", "isManaged": country == "FR", "isCompliant": country == "FR"},
           "SyntheticCase": "REMN-LAB", "SyntheticRecord": key}
    raw.update(extra)
    return {"t": t, "key": key, "raw": raw}
...
def normal_ual(n):
    for i in range(n):
        ...
        yield ual(f"BG-U-{i:07d}", bg_time(i, n), user, op, "AzureActiveDirectory" if op == "UserLoggedIn" else "Exchange" if op == "MailItemsAccessed" else "SharePoint", **extra)
...
def scenarios():
    ...
    specs = [
        ...
        ("S02", "benoit.durand", "2026-09-02T09:10:00Z", "Attachment execution, persistence and Defender alert", "delivery-review.example", "Shipping_NS-002.pdf.exe"),
    ...
    for idx, (sid, user, date, title, domain, filename) in enumerate(specs):
        ...
        mail = {"t": start, "key": sid+"-MAIL-001", "scenario": sid, "label": "malicious_simulation", "from": frm, "to": victim,
                "display": "Marie Lefevre" if sid == "S01" else "Supplier Accounts", "subject": f"[{sid}] Urgent invoice NS-{sid[1:]}", "body": body, "html": html,
                "auth_fail": sid == "S01", "ip": ip, "attachments": [(filename, payload, "application/octet-stream" if sid == "S02" else "text/html")]}
        mails.append(mail)
        mails.append({"t": start+300, "key": sid+"-MAIL-002", "scenario": sid, "label": "victim_response", "from": victim, "to": frm, "subject": "RE: "+mail["subject"], "body": "I opened the requested document. Synthetic exercise reply only.", "reply": mail["key"]})
        descriptions.append({"id": sid, "title": title, "victim": victim, "host": host, "startUTC": iso(start), "ip": ip, "domain": domain,
                             "mailMessageId": f'<{mail["key"]}@remn-lab.example>', "attachment": filename, "attachmentSha256": hashlib.sha256(payload).hexdigest(),
        ...
        def win(suffix, minute, channel, eid, data, target_host=host):
            windows.append({"t": start+int(minute*60), "key": sid+"-WIN-"+suffix, "scenario": sid, "label": "malicious_simulation", "channel": channel,
                            "event_id": eid, "host": target_host, "data": {"User": "NORTHSTAR\\"+user, "SyntheticCase": "REMN-LAB", "ScenarioId": sid, **data}})

        win("001", 2, "Sysmon", 22, {"Image": r"C:\Program Files\Microsoft\Edge\Application\msedge.exe", "QueryName": domain, "QueryResults": ip, "QueryStatus": 0})
        win("002", 3, "Sysmon", 11, {"Image": r"C:\Program Files\Microsoft\Edge\Application\msedge.exe", "TargetFilename": f"C:\\Users\\{user}\\Downloads\\{filename}"})
        win("003", 4, "Sysmon", 3, {"Image": r"C:\Program Files\Microsoft\Edge\Application\msedge.exe", "DestinationIp": ip, "DestinationHostname": domain, "DestinationPort": 443, "Protocol": "tcp"})
        win("004", 6, "Security", 4624, {"TargetUserName": user, "TargetDomainName": "NORTHSTAR", "LogonType": 10, "IpAddress": ip, "TargetLogonId": "0x9a01", "AuthenticationPackageName": "NTLM"})
        command = f"powershell.exe -NoProfile -Command Write-Output 'SYNTHETIC {sid} {filename} https://{domain}/stage'"
        if sid in ("S02", "S04"):
            win("005", 7, "Sysmon", 1, {"Image": r"C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe", "ParentImage": r"C:\Program Files\Microsoft Office\root\Office16\OUTLOOK.EXE", "CommandLine": command, "ProcessGuid": "{"+uid(sid+"proc")+"}", "ProcessId": 7788})
            win("006", 8, "PowerShell", 4104, {"ScriptBlockText": f"Write-Output 'SYNTHETIC {sid}: {filename} https://{domain}/stage'", "ScriptBlockId": uid(sid+"script"), "MessageNumber": 1, "MessageTotal": 1})
            win("007", 14, "Security", 4698, {"SubjectUserName": user, "SubjectDomainName": "NORTHSTAR", "TaskName": "\\Northstar\\Updater-"+sid, "TaskContent": f"<Task><Actions><Exec><Command>powershell.exe</Command><Arguments>-Command Write-Output SYNTHETIC-{sid}</Arguments></Exec></Actions></Task>"})
            win("008", 17, "System", 7045, {"ServiceName": "NSLabUpdater-"+sid, "ImagePath": f"C:\\Users\\Public\\{filename}", "ServiceType": "user mode service", "StartType": "auto start", "AccountName": "LocalSystem"}, "FS-001."+ORG)
            win("009", 19, "Defender", 1116, {"Threat Name": "Trojan:Win32/REMN.Synthetic", "Severity Name": "Severe", "Category Name": "Trojan", "Path": f"file:_C:\\Users\\{user}\\Downloads\\{filename}", "Detection User": "NORTHSTAR\\"+user})
            win("010", 22, "Security", 1102, {"SubjectUserName": user, "SubjectDomainName": "NORTHSTAR", "SubjectLogonId": "0x9a01"})
        if sid == "S04":
            for j in range(30):
                win(f"SPRAY-{j:02d}", -4+j/10, "Security", 4625, {"TargetUserName": user if j<18 else NAMES[j], "TargetDomainName": "NORTHSTAR", "LogonType": 3, "IpAddress": ip, "Status": "0xc000006d", "SubStatus": "0xc000006a"})
            win("ADMIN", 12, "Security", 4732, {"SubjectUserName": user, "TargetUserName": "Administrators", "TargetDomainName": "Builtin", "MemberName": f"CN={user},DC=northstar,DC=example", "MemberSid": "S-1-5-21-111-222-333-1042"})
            win("SHARE", 16, "Security", 5140, {"SubjectUserName": user, "SubjectDomainName": "NORTHSTAR", "IpAddress": ip, "ShareName": "\\\\*\\ADMIN$"}, "FS-001."+ORG)
        for j in range(12):
            row = signin(f"{sid}-ENTRA-FAIL-{j:02d}", start+600+j*12, victim if j<7 else NAMES[j]+"@"+ORG, ip, "NL", 50126, client="Other clients")
            row.update(scenario=sid, label="malicious_simulation"); signins.append(row)
        for j, country in enumerate(("FR", "NL")):
            row = signin(f"{sid}-ENTRA-SUCCESS-{j}", start+60*(1 if j==0 else 15), victim, "198.51.100.10" if j==0 else ip, country,
                         risk="none" if j==0 else "high", client="Browser" if j==0 else "Other clients")
            row.update(scenario=sid, label="benign_context" if j==0 else "malicious_simulation"); signins.append(row)
        ...
        def audit(suffix, minute, operation, **extra):
            ...
            row.update(scenario=sid, label="malicious_simulation"); audits.append(row)

        for j in range(35):
            audit(f"ACCESS-{j:02d}", 17+j*.15, "MailItemsAccessed", RecordType=50, MailboxOwnerUPN=victim, LogonType=2,
        ...
        audit("FORWARD", 26, "New-InboxRule", ObjectId=victim+"\\Invoice routing", Parameters=[{"Name":"Name","Value":"Invoice routing"}, {"Name":"ForwardTo","Value":f"finance@{domain}"}, {"Name":"DeleteMessage","Value":"True"}, {"Name":"SubjectContainsWords","Value":"invoice;payment;facture"}])
        audit("MAILBOX", 35, "Set-Mailbox", ObjectId=victim, Parameters=[{"Name":"ForwardingSmtpAddress","Value":f"smtp:finance@{domain}"},{"Name":"DeliverToMailboxAndForward","Value":"True"}])
        audit("CONSENT", 47, "Consent to application.", workload="AzureActiveDirectory", RecordType=8, ObjectId="Northstar Document Helper "+sid,
        ...
        if sid == "S03":
            audit("ROLE", 55, "Add member to role.", workload="AzureActiveDirectory", RecordType=8, ObjectId=victim,
                  ModifiedProperties=[{"Name":"Role.DisplayName","NewValue":"Global Administrator","OldValue":""}], Target=[{"ID":victim,"Type":5}])
            for j in range(55):
                audit(f"DOWNLOAD-{j}", 60+j*.05, "FileDownloaded", workload="SharePoint", RecordType=6, ObjectId=f"https://documents.northstar.example/finance/invoice-{j}.xlsx")

    # Independent benign controls, with realistic capabilities that should not be high.
    controls = [("HTML-ZIP", "Static HTML report in ZIP", [("report.zip", zip_report(), "application/zip")]),
                ...
                ("BANK-NOTICE", "New bank details following merger", []), ("REPLYTO", "Invoice for completed work", [])]
    for i, (key, subject, att) in enumerate(controls):
        mails.append({"t":epoch("2026-09-02T11:00:00Z")+i*60, "key":"S05-"+key, "scenario":"S05", "label":"benign_control", "from":"reports@vendor.example", "to":"elise.bernard@"+ORG,
                      "subject":"[S05] "+subject, "body":"The agreed work is complete. Following our merger here are our new bank details. Verify with your usual contact number. No urgent action is needed.",
                      "attachments":att, **({"reply_to":"remn.synthetic.accounts@gmail.com"} if key=="REPLYTO" else {})})
    descriptions.append({"id":"S05", "title":"Five benign mail calibration controls", "victim":"elise.bernard@"+ORG, "startUTC":"2026-09-02T11:00:00Z", "expected":"All five controls should stay below high/critical mail risk and core finding priority."})
    # Deliberately tempting, incorrect joins: ground truth explicitly excludes these.
    windows.append({"t":epoch("2026-09-01T09:25:00Z"), "key":"NEG-TENANT-001", "scenario":"NEG-TENANT", "label":"unrelated_negative_control", "channel":"Security", "event_id":1102,
                    "host":"OTHER-WS-001.other-tenant.example", "data":{"SubjectUserName":"alice.martin@other-tenant.example", "SubjectDomainName":"OTHER", "SyntheticCase":"REMN-LAB", "ScenarioId":"NEG-TENANT"}})
    windows.append({"t":epoch("2026-08-20T09:25:00Z"), "key":"NEG-TIME-001", "scenario":"NEG-TIME", "label":"unrelated_negative_control", "channel":"Sysmon", "event_id":22,
                    "host":"WS-001."+ORG, "data":{"User":"NORTHSTAR\\alice.martin", "QueryName":"signin-review.example", "Image":"msedge.exe", "SyntheticCase":"REMN-LAB", "ScenarioId":"NEG-TIME"}})
...
def build_pack(out, counts):
    ...
    manifest = {"generatorVersion":"linked-investigation-v1", "synthetic":True, "seed":20260905, "periodUTC":["2026-08-17T08:00:00Z","2026-09-04T17:00:00Z"], "files":[], "settings":SETTINGS, "scenarios":descriptions}
    truth = []
    ...
    def record_truth(row, file, ordinal, locator):
        if row.get("scenario"):
            truth.append({"scenario":row["scenario"], "label":row["label"], "key":row["key"], "file":file, "ordinal":ordinal, "timeUTC":iso(row["t"]), **locator})
    ...
    with path.open("xb") as fh:
        for i, row in enumerate(merge(normal_mails(n), mail_special), 1):
            ...
            record_truth(row, path.name, i, {"messageId":f'<{row["key"]}@remn-lab.example>', "attachments":[{"name":a[0],"sha256":hashlib.sha256(a[1]).hexdigest()} for a in row.get("attachments",[])]})
    finish(path, i, "mail")
    ratios = {"Security":.6,"Sysmon":.3,"PowerShell":.08,"System":.019,"Defender":.001}
    ...
    for channel, ratio in ratios.items():
        ...
        if count < len(special): raise ValueError(f"Too few {channel} rows for scenarios")
        ...
        with EvtxWriter(path) as w:
            for i, row in enumerate(merge(normal_windows(channel,count-len(special)),special),1):
                ...
                w.add(event_node(i, iso(row["t"]), provider, ch, row["host"], row["event_id"], data, level=3 if row["event_id"] in (4625,1116) else 4), row["t"]*1000)
                record_truth(row,path.name,i,{"eventRecordId":i,"eventId":row["event_id"],"host":row["host"]})
                if i % 100000 == 0: print(f"  {channel}: {i:,} / {count:,}",flush=True)
        finish(path,i,"evtx")
    path=out/"M365-UnifiedAuditLog.csv"
    with path.open("x",encoding="utf-8",newline="") as fh:
        writer=csv.writer(fh);writer.writerow(["CreationDate","UserIds","Operations","AuditData"])
        for i,row in enumerate(merge(normal_ual(counts["ual"]-len(audit_special)),audit_special),1):
            raw=row["raw"];writer.writerow([raw["CreationTime"],raw["UserId"],raw["Operation"],json.dumps(raw,separators=(",",":"))])
...
def write_guide(out):
    ...
    text=["# REMN linked investigation lab", "", "Start with **quick-start** in a new case. Use **large** in a separate server-store case for ingestion, search, rule and correlation load testing.", "",
          ...
          "## Load the evidence", "", "1. Restart the updated REMN application. Create a new case named `Linked lab - quick` or `Linked lab - large`.",
    ...
    for name,m in manifests.items():
        text += [f"### {name}: {m['totalRecords']:,} records / {m['totalBytes']/1e9:.3f} GB", "", "| Evidence file | Records | Size (MB) |", "|---|---:|---:|"]
        for f in m["files"]:
            text.append(f"| [{f['path']}](<{out/name/f['path']}>) | {f['records']:,} | {f['bytes']/1e6:.1f} |")
        text += ["",f"Expected totals: **{sum(f['records'] for f in m['files'] if f['kind']=='mail'):,} mails**, **{sum(f['records'] for f in m['files'] if f['kind']!='mail'):,} events**. M365 and Windows share the Events table.",""]
        ...
        with (out/name/"record-locators.csv").open("w",encoding="utf-8",newline="") as fh:
            fields=["scenario","label","key","file","ordinal","timeUTC","messageId","eventRecordId","eventId","host","auditId","signInId","operation"]
            writer=csv.DictWriter(fh,fieldnames=fields,extrasaction="ignore");writer.writeheader();writer.writerows(truth)
    ...
    text += ["", "The five attack stories are S01, S02, S03, S04 and S06. They include a seed mail, an actual reply with matching In-Reply-To, a DNS lookup for the same domain, a download with the same attachment name, a risky sign-in, mailbox access and forwarding/consent records. S02 and S04 add Outlook-to-PowerShell process creation, a scheduled task, a service installation and a Defender alert. S03 adds role assignment and a file-download burst. S04 adds failed logons, RDP success, group membership and an admin-share event.", "",
             ...
             "## Repeat or validate", "", "Run from the project root:", "", "```powershell", ".\\.venv\\Scripts\\python.exe samples/synthetic/make_linked_lab.py --out samples/generated/another-linked-lab", ".\\.venv\\Scripts\\python.exe samples/synthetic/validate_linked_lab.py samples/generated/another-linked-lab/quick-start --retain-all", ".\\.venv\\Scripts\\python.exe samples/synthetic/validate_linked_lab.py samples/generated/another-linked-lab/large", "```", "", "The generator refuses an existing output directory. Generated evidence is ignored by Git; the reproducible generators are source files.", ""]
    (out/"START-HERE.md").write_text("\n".join(text),encoding="utf-8")
    with (out/"identities.csv").open("w",encoding="utf-8",newline="") as fh:
        writer=csv.writer(fh);writer.writerow(["upn","windowsIdentity","host","expectedCountry"])
        for i,user in enumerate(NAMES,1):writer.writerow([user+"@"+ORG,"NORTHSTAR\\"+user,f"WS-{i:03d}.{ORG}","FR"])
    (out/"investigation-queries.sql").write_text('''-- Run SELECT queries in a server-store case after loading all eight evidence files.
...
def main():
    ...
    if args.out.exists(): parser.error("Output directory already exists; choose a new directory")
    args.out.mkdir(parents=True)
    quick=build_pack(args.out/"quick-start",{"mails":1000,"windows":10000,"ual":2000,"entra":2000})
