...
from services.ingest.pipeline import MailSource, EvtxSource
...
from services.store.writers import MailWriter, EventWriter
...
def validate(pack, retain_all=False):
    ...
    settings={"internal_domains":manifest["settings"]["internalDomains"],"expected_countries":["FR"],"trusted_senders":[],"vip_names":["marie lefevre"],"internal_ips":["10.0.0.0/8"],"businessHours":manifest["settings"]["businessHours"],"weekendDays":[0,6]}
    ctx=ParseContext(internal_domains=settings["internal_domains"],analyze_attachments=True)
    rows_mail=[]; rows_event=[]; validations=[]
    t0=time.monotonic()
    for entry in manifest["files"]:
        ...
        for count,row in enumerate(src,1):
            ...
            if retain_all or count in wanted or count<=500:
                ...
                row["_file"]=path.name;row["_ordinal"]=count
    ...
    reg=StoreRegistry();reg.configure(ROOT/".venv"/"linked-lab-validation-stores")
    store=reg.get(str(uuid.uuid4()))
    try:
        mw=MailWriter(store,1);ew=EventWriter(store,2)
        for row in rows_mail: mw.add(row)
        mw.flush()
        for row in rows_event: ew.add(row)
        ew.flush()
        rules=[]
        ...
        for scenario in manifest["scenarios"]:
            ...
            checks.append({"scenario":scenario["id"],"identity":chain["identity"],"severity":chain["severity"],"score":chain["score"],"artifactLinks":chain["artifactLinks"],"steps":len(chain["steps"])})
        ...
        report={"pack":pack.name,"totalRecords":manifest["totalRecords"],"elapsedSeconds":round(time.monotonic()-t0,2),"files":validations,
                "coreRulesEvaluated":len(rules),"ruleErrors":result["errors"],"findings":result["total"],"byRule":result["byRule"],"chains":checks,
                "benignControls":[{"messageId":r["messageId"],"risk":r["risk"]} for r in benign],
                "knownNegativeControlLimitation":{"sameLocalPartOtherTenantLinkedTo":neg_links,"expected":"No cross-tenant join. Current identity normalizer strips domain scope; this is a deliberate false-correlation control."},
                "ruleAndChainScope":"all parsed rows" if retain_all else "all planted records plus first 500 background records per source; full parser/count/hash validation covers every record"}
