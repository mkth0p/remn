...
PROC = ["C:\\Windows\\System32\\svchost.exe", "C:\\Windows\\explorer.exe", "C:\\Program Files\\Microsoft Office\\root\\Office16\\OUTLOOK.EXE",
        "C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe", "C:\\Windows\\System32\\cmd.exe", "C:\\Users\\Public\\update.exe"]
...
def gen(i: int) -> dict:
    ...
    if r < 0.55:
        ...
        return {"recordId": i, "ts": ts, "eventId": eid, "provider": "Microsoft-Windows-Security-Auditing", "channel": "Security", "computer": host, "level": 0,
                "levelName": "LogAlways", "category": cat, "targetUser": u, "targetDomain": "CORP", "logonType": lt, "logonTypeName": "Network", "ipAddress": ip,
                "authPackage": data["AuthenticationPackageName"], "summary": f"Logon type {lt} as CORP\\{u} from {ip}", "data": data, "raw": json.dumps({"EventID": eid, "EventData": data})}
    if r < 0.80:
        eid = 4625
        ...
        return {"recordId": i, "ts": ts, "eventId": eid, "provider": "Microsoft-Windows-Security-Auditing", "channel": "Security", "computer": host, "level": 0,
                "levelName": "LogAlways", "category": "logon", "targetUser": u, "targetDomain": "CORP", "logonType": 3, "ipAddress": ip, "status": "0xc000006d",
                "subStatus": "0xc000006a", "statusText": "Wrong password", "summary": f"Failed logon Network as CORP\\{u} from {ip} - Wrong password", "data": data,
    ...
    if r < 0.97:
        ...
        return {"recordId": i, "ts": ts, "eventId": eid, "provider": "Microsoft-Windows-Security-Auditing", "channel": "Security", "computer": host, "level": 0,
                "levelName": "LogAlways", "category": "process", "subjectUser": u, "processName": p, "commandLine": cmd, "parentProcessName": data["ParentProcessName"],
                "summary": f"Process {p} by CORP\\{u}", "data": data, "raw": json.dumps({"EventID": eid, "EventData": data})}
    eid = random.choice([7045, 4720, 1102, 4698, 4672, 4104])
    data = {"ServiceName": "PSEXESVC" if eid == 7045 and random.random() < 0.3 else "UpdateSvc", "ImagePath": random.choice(PROC), "SubjectUserName": random.choice(USERS)}
    return {"recordId": i, "ts": ts, "eventId": eid, "provider": "Service Control Manager" if eid == 7045 else "Microsoft-Windows-Security-Auditing", "channel": "System" if eid == 7045 else "Security",
            "computer": host, "level": 4, "levelName": "Information", "category": "persistence", "subjectUser": data["SubjectUserName"], "serviceName": data["ServiceName"],
            "serviceFile": data["ImagePath"], "summary": f"Service installed {data['ServiceName']} -> {data['ImagePath']}", "data": data, "raw": json.dumps({"EventID": eid, "EventData": data})}
...
def main(N: int = 200_000) -> None:
    ...
    s = {"internal_ips": ["10.0.0.0/8"], "businessHours": {"start": 8, "end": 19, "tz": "Europe/Paris"}, "weekendDays": [0, 6], "service_accounts": ["svc_backup", "svc_sql"], "admin_accounts": ["administrator"]}
    ...
    timed("search 4625 from external ip (nin_setting), 3000 rows", lambda: Q.search(st, "events", {"conditions": [{"field": "eventId", "op": "eq", "value": 4625}, {"field": "ipAddress", "op": "nin_setting", "value": "internal_ips"}]}, 3000, settings=s))
    ...
    timed("aggregate ipAddress top 25 (4625)", lambda: Q.aggregate(st, "events", {"conditions": [{"field": "eventId", "op": "eq", "value": 4625}]}, "ipAddress", 25, s))
    ...
    t = time.time()
    res = R.run_rules(st, rules, s, progress=lambda p: print(f"  {p['ruleId']:<45s} {p['findings']:5d} findings {p['ms']:6d} ms", flush=True) if p["findings"] or p["ms"] > 500 else None)
...
def run_mail(n: int) -> None:
    ...
    s = {"internal_domains": ["interne.fr"], "vip_names": ["Marie Lefevre"], "trusted_senders": [],
