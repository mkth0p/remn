...
tool_result = json.dumps({"field": "flags", "total": 11, "distinct": 50, "groups": [
    {"value": "spf_fail", "count": 5}, {"value": "replyto_webmail", "count": 3}, {"value": "att_office_macro", "count": 3},
    {"value": "lexicon_gift_card", "count": 3}, {"value": "sender_lookalike_internal", "count": 0}, {"value": "url_text_href_mismatch", "count": 3}]})
messages = [
...
for chunk in client.chat(**kwargs):
    n += 1
    m = chunk.message
    rec = {"t": round(time.time() - t0, 1), "content": m.content, "thinking": getattr(m, "thinking", None),
           "tool_calls": [{"name": c.function.name, "args": c.function.arguments} for c in (m.tool_calls or [])], "done": chunk.done}
    if chunk.done:
