...
printed too. With "reviewed items only", a story prints when it has a note. The printed stories
of one incident print together, in time order, under one heading ("One intrusion", its label, its
worst severity, its span and hosts), with why they read as one (the bases of the strong and medium
links between them) and how many of its stories are not printed or were left out of it past
twenty. A story is how the
...
On the first day of the APT29 evaluation recording (OTRF), with REMN's own rules, the build reads
one story: pbeesly's, holding the flags of SCRANTON (her RDP session) and NASHUA (her PsExec
sessions) that name no one; the domain controller's and UTICA's own maintenance are leads. Without
the joins above it read five: hers and four host stories, all four high. On the second day
it reads two incidents: dschrute's and mscott's stories with UTICA's and NEWYORK's, joined by
explicit credentials, and kmalone's with the local account on SCRANTON she enabled (one record names both).
...
- A host's flags stand on their rules' measures, read by rule id from `rules/measures.json`: a
  custom rule that reuses a bundled rule's id borrows its measure. A rule measured quiet on the
  clean machines can still fire on a host's own configuration: with the SigmaHQ packs on, the
