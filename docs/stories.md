...
most twenty, before the chains: each with its spine (a row per step: when, its phase with its
worst finding's severity, the step and that finding, the way in and the anchor marked, and why
it is in the story, then how the spine was drawn), its phases in the order they happened, what
marks each (its worst findings, else its first step), the analyst's note (checked against the
