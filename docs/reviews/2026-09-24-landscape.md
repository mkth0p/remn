...
- **Local models are weak at agentic security work, so measuring them is not optional.**
  - Elastic publishes a score per model and feature (1 to 10, 5 or below "not recommended"):
    GPT-OSS-20B 4.01, Qwen 3.6 27B 3.75, Gemma 4 31B 5.77, against 9.23 for the best hosted model.
...
Made alongside this review, each with a regression test that failed before:
