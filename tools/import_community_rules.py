...
def run_tranco(zip_path: Path | None, download: bool, cache_dir: Path, list_id: str | None) -> None:
    """Bundle the top of the Tranco list as the built-in `tranco_10k` reference list (both engines)."""
    if download:
        req = urllib.request.Request(TRANCO_ID_URL, headers={"User-Agent": "remn-rule-import"})
        with urllib.request.urlopen(req, timeout=30) as resp:  # noqa: S310
            list_id = resp.read().decode("ascii").strip()
        cache_dir.mkdir(parents=True, exist_ok=True)
        zip_path = cache_dir / f"tranco-{list_id}.zip"
        if not zip_path.exists():
            print(f"  downloading {TRANCO_URL} (list {list_id})")
            req = urllib.request.Request(TRANCO_URL, headers={"User-Agent": "remn-rule-import"})
            with urllib.request.urlopen(req, timeout=300) as resp, open(zip_path, "wb") as fh:  # noqa: S310
                fh.write(resp.read())
    if zip_path is None:
        raise SystemExit("give --zip <top-1m.csv.zip> or --download")
    with zipfile.ZipFile(zip_path) as zf:
        ...
        for line in zf.read(member).decode("utf-8", "replace").splitlines():
            ...
            dom = dom.strip().lower()
            if dom and rank.strip().isdigit():
                domains.append(dom)
            if len(domains) >= TRANCO_TOP:
                break
...
def main(argv: list[str] | None = None) -> None:
    ...
    ap.add_argument("source", choices=["sigma", "sublime", "tranco", "all"])
    ap.add_argument("--zip", type=Path, help="local archive of the repository (GitHub 'Download ZIP' layout)")
    ap.add_argument("--sha", help="commit the archive corresponds to (recorded in pack.json); for tranco, the list id")
