...
VERSION = "mail-2"
IDENTITY_EVIDENCE = {"internal_spoof", "sender_lookalike_internal", "replyto_lookalike_internal",
                     "sender_homoglyph", "sender_digit_substitution", "sender_subdomain_trick", "display_name_email_mismatch"}
...
def aligned_auth(row: dict[str, Any]) -> bool:
    ...
    return any(auth.get(kind) == "pass" and auth.get(field) and registrable(str(auth[field])) == domain
...
def calibrate_mail(row: dict[str, Any], settings: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    trusted = _user_trusted(str(row.get("fromAddr") or ""), str(row.get("fromDomain") or ""), str(row.get("fromRegistrable") or ""), settings.get("trusted_senders") or [])
    ...
    trust = {"authenticated": auth_ok, "internal": internal, "bulk": "bulk_mailer" in flags or bool(row.get("listId")),
             "notification": notification, "user_trusted": trusted,
    ...
    if expected and not corroborated and not strong_content:
        risk = max(min(risk, 35), int(max_att * .9))
    ...
    out.update(flags=sorted(flags), attachments=attachments, maxAttachmentRisk=max_att, risk=risk,
               assessment={"version": VERSION, "confidence": confidence, "groups": score_groups(flags),
                           "attachmentRisk": max_att, "expectedSender": expected, "limitations": limitations})
    return out
