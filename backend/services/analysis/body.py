...
LEXICON: dict[str, tuple[str, ...]] = {
    "urgency": (
        # EN
        "urgent", "immediately", "as soon as possible", "asap", "right away", "within 24 hours",
        "final notice", "last warning", "act now", "expires today", "time sensitive", "deadline",
        "your account will be", "suspended", "locked", "deactivated", "terminated", "unusual activity",
        "unauthorized", "security alert", "action required", "verify your", "confirm your",
        # FR
        "urgent", "immédiatement", "dès que possible", "au plus vite", "sous 24h", "sous 24 heures",
        "dernier rappel", "dernier avertissement", "dernière relance", "expire aujourd'hui",
        "votre compte sera", "suspendu", "bloqué", "désactivé", "activité inhabituelle",
        "non autorisé", "alerte de sécurité", "action requise", "vérifiez votre", "confirmez votre",
        "mise à jour obligatoire", "régularisation", "mise en demeure",
    ),
    "financial": (
        "wire transfer", "bank transfer", "bank details", "iban", "swift", "account number",
        "routing number", "invoice", "overdue", "payment", "pay now", "outstanding balance",
        "purchase order", "remittance", "change of bank", "new bank account", "w-9", "ach",
        "virement", "virement bancaire", "coordonnées bancaires", "rib", "facture", "impayé",
        "paiement", "règlement", "solde", "bon de commande", "changement de rib", "nouveau rib",
        "relevé", "remboursement", "refund", "tax refund", "impôts", "taxe",
    ),
    "gift_card": (
        "gift card", "gift cards", "itunes", "google play card", "steam card", "amazon card",
        "scratch the card", "carte cadeau", "cartes cadeaux", "carte prépayée", "code de la carte",
        "transcash", "pcs", "neosurf", "paysafecard",
    ),
    "credentials": (
        "password", "passcode", "username", "login", "log in", "sign in", "credentials",
        "one-time code", "verification code", "2fa", "mfa", "authenticator", "otp", "pin",
        "mot de passe", "identifiant", "identifiants", "connexion", "se connecter", "code de vérification",
        "code à usage unique", "double authentification", "authentification",
    ),
    "authority": (
        "ceo", "cfo", "chief executive", "managing director", "president", "board", "legal department",
        "hr department", "it department", "helpdesk", "help desk", "system administrator", "microsoft",
        "office 365", "it support", "administrator", "police", "gendarmerie", "tribunal", "huissier",
        "direction générale", "directeur général", "pdg", "dg", "daf", "service juridique",
        "service informatique", "service rh", "ressources humaines", "administrateur",
    ),
    "secrecy": (
        "confidential", "do not share", "keep this between us", "discreet", "discretion", "private matter",
        "don't tell", "confidentiel", "ne pas divulguer", "entre nous", "discret", "discrétion",
        "ne parlez", "n'en parlez", "personnel",
    ),
    "availability": (
        "in a meeting", "can't talk", "cannot talk", "can't call", "unreachable", "on a call",
        "reply by email only", "text me", "send me your number", "are you available", "are you at your desk",
        "en réunion", "je ne peux pas", "injoignable", "répondez par mail", "envoyez-moi votre numéro",
        "êtes-vous disponible", "es-tu disponible", "tu es là", "êtes-vous au bureau",
    ),
    "delivery": (
        "package", "parcel", "shipment", "delivery", "tracking number", "customs fee", "undelivered",
        "colis", "livraison", "numéro de suivi", "frais de douane", "non livré", "en attente de livraison",
        "reprogrammer", "reschedule",
    ),
    "document_lure": (
        "shared a document", "shared a file", "view document", "open document", "review the attached",
        "see attached", "please find attached", "attached invoice", "attached file", "voicemail",
        "voice message", "fax received", "scanned document", "encrypted message", "secure message",
        "a partagé un document", "a partagé un fichier", "voir le document", "ouvrir le document",
        "ci-joint", "veuillez trouver ci-joint", "pièce jointe", "message vocal", "messagerie vocale",
        "document numérisé", "message sécurisé", "message chiffré",
    ),
}
