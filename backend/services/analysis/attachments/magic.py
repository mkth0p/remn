...
def analyze_name(name: str | None) -> dict[str, Any]:
    ...
    if len(parts) >= 3 and parts[-1] in DANGEROUS_EXT and parts[-2] in DANGEROUS_EXT and DANGEROUS_EXT.get(parts[-1]) in EXEC_CATEGORIES:
        ...
        len(parts) >= 3
        and DANGEROUS_EXT.get(parts[-1]) in EXEC_CATEGORIES
        and parts[-2] in ("pdf", "doc", "docx", "xls", "xlsx", "jpg", "png", "txt", "html", "ppt", "pptx")
    ):
        flags.append("double_extension")
...
_EQUIVALENT: dict[str, set[str]] = {
    ...
    "docx": {"docx", "docm", "dotx", "dotm", "ooxml"},
    "xlsx": {"xlsx", "xlsm", "xltx", "xltm", "xlam", "xlsb", "ooxml"},
    "pptx": {"pptx", "pptm", "potx", "potm", "ppsx", "ppsm", "ooxml"},
    "doc": {"doc", "dot", "wiz"},
    "xls": {"xls", "xlt", "xla"},
    "ppt": {"ppt", "pot", "pps"},
    "zip": {"zip", "zipx"},
    ...
    "eml": {"eml", "txt", "mht", "mhtml"},
    "msg": {"msg", "ole"},
    "ole": {"doc", "xls", "ppt", "msg", "ole", "pub", "mdb"},
    "gz": {"gz", "tgz", "gzip"},
    "exe": {"exe", "dll", "scr", "com", "pif", "cpl", "sys", "ocx", "xll", "drv"},
    "tar": {"tar"},
    "jar": {"jar", "zip"},
    "apk": {"apk"},
    "svg": {"svg", "xml"},
    "xml": {"xml", "svg", "xhtml", "rels", "config", "ps1xml"},
    "mp3": {"mp3"},
    "mp4": {"mp4", "m4v", "mov"},
    "png": {"png"},
    "gif": {"gif"},
