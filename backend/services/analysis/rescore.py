    ...
    with store.lock:
        ...
        try:
            while cursor_id < last_id:
                ...
                con.register("_mail_calibration", pa.Table.from_pylist(updates, schema=pa.schema([
                    ("id", pa.int64()), ("risk", pa.int32()), ("flags", pa.list_(pa.string())),
                    ("attachments", pa.string()), ("maxAttachmentRisk", pa.int32()), ("assessment", pa.string())])))
                try:
                    con.execute('UPDATE mails SET risk=u.risk, flags=u.flags, attachments=u.attachments, "maxAttachmentRisk"=u."maxAttachmentRisk", assessment=u.assessment FROM _mail_calibration u WHERE mails.id=u.id')
                finally:
                    con.unregister("_mail_calibration")
                # Update the normalized attachment table as well as the mail JSON.
                for r in result["rows"]:
                    for a in r["attachments"]:
                        con.execute('UPDATE attachments SET risk=?, flags=?, details=? WHERE "mailId"=? AND name IS NOT DISTINCT FROM ? AND sha256 IS NOT DISTINCT FROM ?',
