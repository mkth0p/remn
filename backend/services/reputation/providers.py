...
class URLhaus(Provider):
    ...
    def lookup(self, kind: str, value: str) -> Verdict:
        ...
        if kind == "url":
            tags = d.get("tags") or []
            return Verdict(self.name, kind, value, "malicious", 95, tags=[str(t) for t in tags],
                           details={"threat": d.get("threat"), "urlStatus": d.get("url_status"), "dateAdded": d.get("date_added"),
                                    "blacklists": d.get("blacklists")}, link=d.get("urlhaus_reference"))
        urls = d.get("urls") or []
        ...
        return Verdict(self.name, kind, value, verdict, 90 if online else 60, tags=tags,
                       details={"urlCount": d.get("url_count") or len(urls), "online": online, "firstSeen": d.get("firstseen"),
                                "blacklists": d.get("blacklists"), "sample": [u.get("url") for u in urls[:5]]},
...
class MalwareBazaar(Provider):
    ...
    def lookup(self, kind: str, value: str) -> Verdict:
        ...
        return Verdict(self.name, kind, value, "malicious", 98, tags=[str(t) for t in (data.get("tags") or [])],
                       details={"fileName": data.get("file_name"), "fileType": data.get("file_type"), "signature": data.get("signature"),
                                "firstSeen": data.get("first_seen"), "sha256": data.get("sha256_hash"), "deliveryMethod": data.get("delivery_method")},
...
class ThreatFox(Provider):
    ...
    def lookup(self, kind: str, value: str) -> Verdict:
        with _client(self.timeout) as c:
            r = c.post("https://threatfox-api.abuse.ch/api/v1/", json={"query": "search_ioc", "search_term": value, "exact_match": True},
                       headers={"Auth-Key": self.key})
            r.raise_for_status()
            d = r.json()
        status = d.get("query_status")
        ...
        return Verdict(self.name, kind, value, "malicious", int(best.get("confidence_level") or 80),
                       tags=[str(t) for t in (best.get("tags") or [])] + [best.get("malware_printable") or ""],
                       details={"threatType": best.get("threat_type"), "malware": best.get("malware_printable"),
                                "firstSeen": best.get("first_seen"), "reporter": best.get("reporter"), "count": len(items)},
...
class VirusTotal(Provider):
    ...
    def lookup(self, kind: str, value: str) -> Verdict:
        ...
        details: dict[str, Any] = {"malicious": mal, "suspicious": sus, "harmless": stats.get("harmless"), "undetected": stats.get("undetected"),
                                   "reputation": attrs.get("reputation"), "categories": attrs.get("categories")}
        if kind == "ip":
        ...
        if kind == "hash":
            details.update({"names": (attrs.get("names") or [])[:5], "typeDescription": attrs.get("type_description"),
...
class AbuseIPDB(Provider):
    ...
    def lookup(self, kind: str, value: str) -> Verdict:
        with _client(self.timeout) as c:
            r = c.get("https://api.abuseipdb.com/api/v2/check", params={"ipAddress": value, "maxAgeInDays": 90, "verbose": ""},
        ...
        return Verdict(self.name, kind, value, verdict, conf, tags=tags,
                       details={"totalReports": d.get("totalReports"), "distinctUsers": d.get("numDistinctUsers"), "lastReported": d.get("lastReportedAt"),
                                "isp": d.get("isp"), "org": d.get("isp"), "country": d.get("countryCode"), "domain": d.get("domain"),
...
class GreyNoise(Provider):
    ...
    def lookup(self, kind: str, value: str) -> Verdict:
        ...
        return Verdict(self.name, kind, value, verdict, {"malicious": 85, "benign": 5}.get(cls), tags=tags,
...
class IPInfo(Provider):
    ...
    def lookup(self, kind: str, value: str) -> Verdict:
        ...
        for k in ("vpn", "proxy", "tor", "hosting", "relay"):
            if privacy.get(k):
                tags.append(k)
        return Verdict(self.name, kind, value, "unknown", None, tags=tags,
                       details={"country": d.get("country"), "region": d.get("region"), "city": d.get("city"), "org": org, "asn": asn,
                                "hostname": d.get("hostname"), "timezone": d.get("timezone"), "anycast": d.get("anycast")},
...
class RDAP(Provider):
    ...
    def lookup(self, kind: str, value: str) -> Verdict:
        ...
        return Verdict(self.name, kind, value, verdict, score, tags=tags,
                       details={"domain": dom, "registered": reg, "ageDays": age_days, "expires": events.get("expiration"),
                                "lastChanged": events.get("last changed"), "registrar": registrar, "status": (d.get("status") or [])[:8],
...
class SafeBrowsing(Provider):
    ...
    def lookup(self, kind: str, value: str) -> Verdict:
        body = {"client": {"clientId": "remn-forensic", "clientVersion": "0.1"},
                "threatInfo": {"threatTypes": ["MALWARE", "SOCIAL_ENGINEERING", "UNWANTED_SOFTWARE", "POTENTIALLY_HARMFUL_APPLICATION"],
                               "platformTypes": ["ANY_PLATFORM"], "threatEntryTypes": ["URL"], "threatEntries": [{"url": value}]}}
        with _client(self.timeout) as c:
